...
/**
 * 지급계좌 PII(계좌번호·예금주명) 필드 암호화 컨버터 (adapter/out/persistence 배치, 도메인 무오염).
 *
 * <p>AES-GCM(256) 봉투 암호화. 저장 형식은 {@code enc:v1:} 접두 + {@code Base64(IV || ciphertext+tag)}.
 * 키는 env {@code PAYOUT_ENC_KEY}(Base64 32바이트 = AES-256)에서 로드한다 — JWT_SECRET 과 동일하게
 ...
 * 이전의 평문으로 간주하고 그대로 반환한다. 기존 행을 일괄 재기록하지 않으며, 해당 행이 다시 저장될 때
...
public class PayoutFieldEncryptionConverter implements AttributeConverter<String, String> {
    ...
    private static final String ENV_KEY = "PAYOUT_ENC_KEY";
    ...
    public String convertToDatabaseColumn(String attribute) {
        ...
        try {
            ...
            throw new IllegalStateException("Payout 지급계좌 필드 암호화 실패", e);
        }
    }
    ...
    public String convertToEntityAttribute(String dbData) {
        ...
        try {
            ...
            throw new IllegalStateException("Payout 지급계좌 필드 복호화 실패", e);
        }
    }
}
