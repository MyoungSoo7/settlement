 ...
 *
 * <p>멱등: (consumer_group, event_id) PK. 뷰 컨슈머와 그룹을 분리해 한쪽 실패가 다른 쪽 처리를
 * 막지 않는다(뷰 갱신은 성공했는데 조정만 DLT 로 빠지는 경우 등).
 *
 * <p>실패 정책(fail-loud): 정산 미존재(환불 이벤트가 정산 생성보다 먼저 도착)나 DONE 정산
 * (지급 완료 후 환불 — 도메인이 직접 차감을 금지)은 예외를 전파해 재시도 후 DLT 로 보낸다.
 * 조용히 넘기면 "환불 미반영 전액 지급"이 재발하므로 DLT 알림·replay 로 드러나게 한다.
 *
 * <p>금액 해석: 신규 페이로드의 {@code refundAmount}(건별 delta)를 사용한다. 구버전 페이로드
 * (누적 {@code refundedAmount} 만 존재)는 정산의 기반영 누적치와의 차이로 delta 를 복원한다
 * — DLT replay 시에도 이중 차감 없이 수렴한다.
 */
@Component
@ConditionalOnProperty(name = "app.kafka.enabled", havingValue = "true")
public class PaymentRefundedSettlementAdjustConsumer {

    private static final Logger log = LoggerFactory.getLogger(PaymentRefundedSettlementAdjustConsumer.class);
    ...
    public void onPaymentRefunded(ConsumerRecord<String, String> record, Acknowledgment ack) {
        ...
        if (eventId == null) {
            log.warn("Skipping refund-adjust event without event_id header. topic={}, offset={}",
        ...
        if (processedEventRepository.existsById(key)) {
            log.info("Refund-adjust event already processed, skipping. eventId={}", eventId);
            ack.acknowledge();
            return;
        }
        ...
        if (delta == null || delta.signum() <= 0) {
            // 이미 반영된 레거시 재전송 등 — 조정 없이 처리 완료로 기록해 poison 재시도를 막는다.
     ...
     * "이벤트 누적치 − 정산 기반영 누적치"로 복원한다. 0 이하면 이미 반영된 것이므로 스킵 신호(null 대신 0).
     */
