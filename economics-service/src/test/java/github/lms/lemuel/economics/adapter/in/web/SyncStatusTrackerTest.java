...
class SyncStatusTrackerTest {
    ...
    void tryStartClaimsAndBlocks() {
        ...
        assertThat(tracker.current().job()).isEqualTo("all:2026-01-01~2026-06-30");
        assertThat(tracker.current().startedAt()).isNotNull();

        assertThat(tracker.tryStart("CPI:2026-01-01~2026-06-30")).isFalse();
    }
    ...
    void complete() {
        ...
        assertThat(tracker.tryStart("CPI")).isTrue();
    }
    ...
    void fail() {
        ...
        assertThat(tracker.current().error()).isEqualTo("ECOS timeout");
