...
/**
 * 부팅 시 + 매월 1회 indicator_values 연별 파티션 런웨이를 굴린다 — economics-service(public).
 *
 * <p>{@code indicator_values} 는 지표 시계열이라 <b>연별</b> RANGE 파티션이다
 * ({@code ensure_indicator_value_partition} 은 years_ahead 인자). 유한 구간(2028)까지만 선생성돼 그 이후
 * 삽입은 DEFAULT 파티션으로 흘러들어 프루닝·리텐션 이점이 사라진다. 이 러너가 부팅 때 함수를 호출해 미래
 * 파티션을 굴린다.
 *
...
public class PartitionMaintenanceRunner implements ApplicationRunner {
    ...
    public PartitionMaintenanceRunner(
            ...
            // 네이티브 SQL 은 hibernate.default_schema 를 무시하므로 함수명을 명시 한정한다(economics=public).
    ...
    private void ensurePartitions() {
        try {
            Integer created = jdbcTemplate.queryForObject(
                    "SELECT " + schema + ".ensure_indicator_value_partition(?)", Integer.class, yearsAhead);
            log.info("[PartitionMaintenance] ensure_indicator_value_partition({}) 완료: 신규 파티션 {}개", yearsAhead, created);
        } catch (RuntimeException e) {
            log.warn("[PartitionMaintenance] ensure_indicator_value_partition 실패 — 유지보수 스킵 (fail-open): {}", e.getMessage());
        }
    }
}
