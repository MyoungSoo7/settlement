...
/**
 * ECOS 수집 트리거(/admin/economics/**) 공유 시크릿 게이트.
 *
 * <p>X-Internal-Api-Key 헤더가 {@code app.economics.internal-api-key} 와 일치해야 통과.
...
public class AdminApiKeyFilter extends OncePerRequestFilter {
    ...
    public AdminApiKeyFilter(@Value("${app.economics.internal-api-key:}") String apiKey) {
        this.apiKey = apiKey;
    }

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !request.getRequestURI().startsWith("/admin/economics/");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("app.economics.internal-api-key 미설정 — /admin/economics/** 무게이팅 통과 (로컬 전용, 운영 설정 필수)");
