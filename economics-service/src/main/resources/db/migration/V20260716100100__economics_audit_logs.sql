-- V20260716100100: economics-service 감사 로그(audit_logs) 신설 — 3인 DB 리뷰(R2) 지적 반영
--
-- 설계 근거:
--   economics-service 는 공개 read-only 위성이지만 `/admin/**` 수집 트리거(ECOS 동기화)와 시드 적재
--   경로가 지표 카탈로그·관측치 시계열을 자체 DB(public)에 갱신한다. 현재 자체 DB 에 감사 추적 테이블이
--   전무해 "누가 언제 어떤 지표의 ECOS 수집/시드를 실행했나"를 포스트모템에서 재구성할 수 없다는 것이
--   리뷰 지적. → order-service V34 / company-service V20260715150000 와 동일한 shared-common audit
--     표준 스키마를 economics 자체 DB(public)에 신설한다.
--
-- E3 표준(감사성 강화형) 채택:
...
--
-- ★ 주의(스키마 선행): 본 마이그레이션은 테이블만 만든다. economics 는 shared-common 미의존 위성
...
CREATE TABLE IF NOT EXISTS audit_logs (
    ...
    resource_type   VARCHAR(50),               -- Indicator / IndicatorValue / EcosSync ...
    resource_id     VARCHAR(64),               -- 문자열 보관 (지표코드 등)
...
COMMENT ON TABLE audit_logs IS 'economics 민감 작업(ECOS 수집 트리거·시드 적재·지표 카탈로그 갱신) 감사 추적. append-only + 월별 파티션. 기록 배선 완료 — 자체 audit 어댑터(RecordAuditPort→AuditRecordingAdapter)가 admin 행위를 실적재.';
