...
- **커밋**: `main` 직접 push **금지**(보호 브랜치). 제출물 플러그인은 원격 실행을 위해 커밋 대상(재생성 산출물은 .gitignore 유지).
...
`/api/notifications/stream` 알림 푸시 SSE 만 gateway 라우팅 + compose 배선. 정본 [`docs/inflearn/sse.md`](docs/inflearn/sse.md))을
...
├── settlement-service/           # 💰 Settlement (8082, settlement_db, standalone) — settlement·payout·ledger·tax·chargeback·pgreconciliation·recovery·closing·report·recon·integrity·idempotency
