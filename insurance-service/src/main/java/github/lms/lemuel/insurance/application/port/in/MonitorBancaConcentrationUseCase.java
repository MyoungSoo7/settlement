 ...
 *
 * <p>당해연도 방카 신계약(effective_date 기준)을 은행 × 원수사로 집계해
 * 특정 원수사 비중이 상한(25%)을 초과한 은행을 찾아낸다.
...
public interface MonitorBancaConcentrationUseCase {
    ...
    /**
     * @param aggregatedPairs 집계된 (은행, 원수사) 조합 수
     * @param violations      상한 초과 위반 목록
     */
    record BancaMonitorResult(int aggregatedPairs, List<BancaRuleViolation> violations) {
    }
}
