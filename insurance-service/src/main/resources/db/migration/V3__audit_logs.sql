...
--
-- insurance-service 는 루트 컴포넌트 스캔으로 shared-common 의 AuditLogJpaEntity(@Table audit_logs) 를
-- 포함하므로, 자체 DB 에도 동일 테이블이 필요하다 (ddl-auto=validate 정합).
--
-- ★ card V3 복사 — 처음부터 파티션드 부모 + append-only 트리거 + 유지보수 함수 2종.
--   (organization 처럼 비파티션 → 나중에 전환하는 리네임/이관 단계 없이, 처음부터 최종 표준)
...
-- 2) 월별 파티션 2026_01 ~ 2028_12 + DEFAULT.
