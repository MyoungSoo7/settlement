-- V2: insurance-service 자체 DB(lemuel_insurance) 용 Outbox / 멱등 추적 테이블 + ShedLock
--
...
-- (card V2 / organization / investment / loan V4 와 동일 스키마).
--
-- ★ card V2 복사 — 인덱스/제약 접두어를 insurance 로 변경.
--   처음부터 최종 강화형(occurred_at NOT NULL, claimed_at/claimed_by, 백필 불필요).

CREATE TABLE IF NOT EXISTS outbox_events (
    id                BIGSERIAL PRIMARY KEY,
    aggregate_type    VARCHAR(50)  NOT NULL,      -- 예: "Insurance" (KafkaOutboxPublisher 토픽 접두어로 소문자화)
    aggregate_id      VARCHAR(64)  NOT NULL,
    event_type        VARCHAR(100) NOT NULL,      -- 예: "InsurancePolicyIssued" → lemuel.insurance.policy_issued
    event_id          UUID         NOT NULL,      -- 전역 고유 — 컨슈머 측 멱등 키 (3단 멱등성 L1)
    ...
    CONSTRAINT chk_insurance_outbox_status CHECK (status IN ('PENDING', 'PUBLISHED', 'FAILED'))
);

CREATE INDEX IF NOT EXISTS idx_insurance_outbox_status_created
    ON outbox_events (status, created_at)
    WHERE status IN ('PENDING', 'FAILED');

CREATE UNIQUE INDEX IF NOT EXISTS uq_insurance_outbox_event_id
    ON outbox_events (event_id);

CREATE INDEX IF NOT EXISTS idx_insurance_outbox_aggregate
    ON outbox_events (aggregate_type, aggregate_id);

CREATE INDEX IF NOT EXISTS idx_insurance_outbox_pending_claim
    ON outbox_events (created_at, claimed_at)
    WHERE status = 'PENDING';

-- 사건 시각 기준 조회(지연 측정·기간별 재처리)를 위한 인덱스.
CREATE INDEX IF NOT EXISTS idx_insurance_outbox_occurred_at
    ON outbox_events (occurred_at);

-- PUBLISHED outbox 정리 스캔용 부분 인덱스 (위 idx_insurance_outbox_status_created 는 PENDING/FAILED 전용)
CREATE INDEX IF NOT EXISTS idx_insurance_outbox_published_prune
...
CREATE INDEX IF NOT EXISTS idx_insurance_processed_events_processed_at
...
COMMENT ON TABLE shedlock IS 'ShedLock distributed lock — @SchedulerLock annotation 의 backing store (월 마감 배치·Outbox 폴러 HA)';
