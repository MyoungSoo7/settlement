...
이커머스 + 정산(Settlement) MSA 플랫폼의 전체 기능 명세. 13개 마이크로서비스 + API Gateway 로 구성된
...
| 항목 | 내용 |
|------|------|
| 도메인 | 주문·결제·정산·선정산/기업대출·투자·계정계·재무제표·경제지표·기업뉴스평판·운영관제·주식시세·AI챗봇·공공데이터·조직/멤버십 |
| 서비스 수 | 13개 마이크로서비스 + API Gateway |
...
| 토픽 | 프로듀서 | 주요 컨슈머 |
|------|----------|-------------|
| `lemuel.payment.captured` / `.refunded` | order | settlement(프로젝션·정산 생성/역정산) |
...
| `lemuel.settlement.confirmed` | settlement | loan · investment · account |
...
- **배포**: Docker Compose(로컬, DB-per-service PG 13종+ES+Redpanda+서비스+gateway), Kubernetes(운영), Flyway 마이그레이션.
