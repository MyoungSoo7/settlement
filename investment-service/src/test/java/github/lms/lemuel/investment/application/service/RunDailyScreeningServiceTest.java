...
class RunDailyScreeningServiceTest {
    ...
    private final LoadStockRecommendationPort loadRecommendation = mock(LoadStockRecommendationPort.class);
    ...
    private RunDailyScreeningService service(UniverseEntry... universe) {
        ...
        return new RunDailyScreeningService(screen, loadCloses, loadRecommendation, props);
    }
    ...
    void 새_종가면_시세기준일로_위임() {
        ...
        when(loadRecommendation.loadLatestDate()).thenReturn(Optional.of(MON));
    ...
    void 이미_최신이면_스킵() {
        when(loadCloses.loadRecentYear("005930")).thenReturn(List.of(close(MON), close(TUE)));
        when(loadRecommendation.loadLatestDate()).thenReturn(Optional.of(TUE));
    ...
    void 저장_세트가_없으면_스크리닝() {
        ...
        when(loadRecommendation.loadLatestDate()).thenReturn(Optional.empty());
