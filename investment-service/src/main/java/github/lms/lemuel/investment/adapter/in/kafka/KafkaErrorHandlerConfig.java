...
/**
 * 투자 서비스 Kafka 컨슈머 에러 핸들링 설정(settlement {@code KafkaErrorHandlerConfig} 동형 배선).
 *
 * <p>배경(감사 지적): 기존엔 Spring Kafka 기본 에러핸들러({@code FixedBackOff(0, 9)})만 있어
 * 재시도 소진 후 메시지를 조용히 skip = 사실상 유실했다. settlement.confirmed 는 투자 재원의 유일한
 * 원천이므로 유실 시 재원 프로젝션이 영구 결손된다.
 *
 * <p>핵심 동작:
 * <ol>
 ...
 * 진단 헤더를 함께 부여해 사후 추적·멱등 replay 를 보장한다. 메트릭 {@code investment.kafka.dlt.published}
...
public class KafkaErrorHandlerConfig {
    ...
    @Bean
    public ConsumerFactory<String, String> investmentConsumerFactory(
            @Value("${spring.kafka.consumer.group-id:lemuel-investment}") String groupId) {
        Map<String, Object> props = new HashMap<>();
     ...
     */
    @Bean
    public DeadLetterPublishingRecoverer investmentDeadLetterRecoverer(KafkaTemplate<String, String> dltKafkaTemplate) {
        Counter dltPublishedCounter = Counter.builder("investment.kafka.dlt.published")
     ...
     */
    @Bean
    public DefaultErrorHandler investmentKafkaErrorHandler(DeadLetterPublishingRecoverer investmentDeadLetterRecoverer) {
        FixedBackOff backOff = new FixedBackOff(RETRY_INTERVAL_MS, MAX_RETRIES);
        DefaultErrorHandler handler = new DefaultErrorHandler(investmentDeadLetterRecoverer, backOff);
        ...
        Counter retryCounter = Counter.builder("investment.kafka.retry")
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> kafkaListenerContainerFactory(
            ConsumerFactory<String, String> investmentConsumerFactory,
            DefaultErrorHandler investmentKafkaErrorHandler) {
        ConcurrentKafkaListenerContainerFactory<String, String> factory =
                new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(investmentConsumerFactory);
