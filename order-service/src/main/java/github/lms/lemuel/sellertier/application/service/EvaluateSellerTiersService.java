 ...
 * 운영자는 어디까지 됐는지 모른다. 등급은 셀러마다 독립적인 사실이므로 건별로 확정하고, 실패는
 * 카운터로 드러내 다음 주기가 다시 집게 한다.
 *
...
public class EvaluateSellerTiersService implements EvaluateSellerTiersUseCase {
    ...
    public EvaluateSellerTiersService(LoadSellerNetSalesPort netSalesPort, LoadTierAssignmentPort loadPort,
                                      SaveTierAssignmentPort savePort, SaveTierHistoryPort historyPort,
                                      SellerTierPolicy policy, int missThreshold, int guardMonths) {
    ...
    public TierEvaluationReport evaluate(LocalDate today, boolean dryRun, int limit) {
        ...
        for (SellerNetSales seller : sellers) {
            try {
                ...
                if (!dryRun) {
                    persist(assignment, decision, seller, today, from);
                }
    ...
    private void persist(TierAssignment assignment, TierDecision decision, SellerNetSales seller,
                         LocalDate today, SellerTierGrade from) {
        ...
        if (decision.outcome() == TierOutcome.GUARDED) {
            return;   // 미달 카운트만 남기고 등급 이력은 남기지 않는다(등급이 안 바뀌었으므로)
        }
        historyPort.append(new TierHistoryEntry(
                seller.sellerId(), from, assignment.getTier(),
                decision.outcome() == TierOutcome.PROMOTED
                        ? TierChangeReason.AUTO_PROMOTION : TierChangeReason.AUTO_DEMOTION,
                seller.net12m(), today.minusMonths(12), today, "SYSTEM", decision.reason()));
    }

    /** 관리자 지정 시 붙일 보호 기간(개월) — 유예 없이 지정하면 다음 평가에서 곧바로 뒤집힌다. */
