...
public class TossPaymentService {
    ...
    public PaymentDomain confirmTossPayment(Long dbOrderId, String paymentKey, String tossOrderId,
        ...
        String key = effectiveKey(idempotencyKey, paymentKey);
        Optional<PaymentDomain> replayed = replay(key);
        if (replayed.isPresent()) {
            log.info("멱등 승인 replay: key={}, paymentId={}", LogSafe.of(key), replayed.get().getId());
            return replayed.get();
        }
    ...
    private static String effectiveKey(String idempotencyKey, String paymentKey) {
        return (idempotencyKey == null || idempotencyKey.isBlank()) ? paymentKey : idempotencyKey;
    }
