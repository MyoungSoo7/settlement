...
/**
 * 부팅 시 + 매월 1회 월별 파티션 런웨이를 굴린다 — ai-service(public).
 *
 * <p>두 개의 파티션드 테이블을 유지한다: {@code audit_logs}(ensure_audit_log_partition) 와
 * {@code chat_messages}(ensure_chat_message_partition). 월별 RANGE 파티션은 유한 구간(2028-12)까지만
...
public class PartitionMaintenanceRunner implements ApplicationRunner {
    ...
    public PartitionMaintenanceRunner(
            ...
            // 네이티브 SQL 은 hibernate.default_schema 를 무시하므로 함수명을 명시 한정한다(ai=public).
    ...
    private void ensurePartitions() {
        ...
        ensure("ensure_chat_message_partition", monthsAhead);
    }
