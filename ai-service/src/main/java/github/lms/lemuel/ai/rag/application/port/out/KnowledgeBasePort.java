...
public interface KnowledgeBasePort {
    ...
    /**
     * 적재된 문서 전량 (출처 오름차순, 청크 본문은 제외).
     *
     * <p>운영자가 "지금 챗봇이 무엇을 근거로 말하는가"를 확인할 유일한 수단이다. 적재 API 만 있고
     * 목록이 없으면 매니페스트에서 빠진 문서가 지식베이스에 <b>영원히 남는다</b> — 삭제는 출처를
     * 알아야 할 수 있는데, 무엇이 들어 있는지 알 방법이 없기 때문이다(고아 문서 회수 불가).
     *
     * <p>청크 본문을 싣지 않는 이유: 목록은 대조·회수용이고, 본문 확인은 검색 API 의 몫이다.
     * 문서 수만큼의 전문을 한 응답에 담으면 200KB × N 이 된다.
     */
