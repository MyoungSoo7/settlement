...
/**
 * 영수증 OCR 설정 (prefix=app.card.receipt-ocr).
 *
 * @param apiKey          Gemini 키(x-goog-api-key). 미설정이면 영수증 첨부는 503 (무폴백, ADR 0036)
 ...
 * @param required        전면 강제 플래그 (기본 false) — 켜면 영수증 미첨부 보고서는 승인 자체가
 *                        거절된다(422). 끄면 점진 도입(첨부된 경우에만 MATCHED 요구)
 */
@ConfigurationProperties(prefix = "app.card.receipt-ocr")
public record ReceiptOcrProperties(String apiKey, String model, String baseUrl,
