...
/**
 * account 서비스 Kafka 컨슈머 에러 핸들링 설정(loan/settlement/investment {@code KafkaErrorHandlerConfig} 동형 배선).
 *
 * <p>배경(감사 지적): 기존엔 Spring Kafka 기본 에러핸들러({@code FixedBackOff(0, 9)})만 있어
 * 재시도 소진 후 메시지를 조용히 skip = 사실상 유실했다. account 는 전사 GL 의 유일한 집계자이고
 * 소비 전용이라 재발행 경로가 없으므로, 분개 이벤트 유실은 시산표 영구 결손으로 직결된다.
 *
 * <p>핵심 동작:
 * <ol>
 ...
 * 진단 헤더를 함께 부여해 사후 추적·멱등 replay 를 보장한다. 메트릭 {@code account.kafka.dlt.published}
 * 로 알람 임계를 걸 수 있다. producer 는 {@code acks=all}+idempotence 로 DLT 손실을 막는다.
 *
 * <p>주의: DLT publish 전용 프로듀서는 소비-전용 가드레일(이벤트 발행 금지)의 예외가 아니다 —
 * 비즈니스 이벤트 발행이 아니라 실패 record 의 사본 격리이며, Outbox 머시너리도 쓰지 않는다.
 */
@Configuration
@EnableKafka
...
public class KafkaErrorHandlerConfig {
    ...
    /** 리스너 컨테이너 동시성 — 파티션 수 상한. 멱등 2단 방어+자연키 UNIQUE 로 파티션 간 병렬 소비가 안전하다. */
    ...
    public ConsumerFactory<String, String> accountConsumerFactory(
            @Value("${spring.kafka.consumer.group-id:lemuel-account}") String groupId) {
        Map<String, Object> props = new HashMap<>();
     ...
     */
    @Bean
    public DeadLetterPublishingRecoverer accountDeadLetterRecoverer(KafkaTemplate<String, String> dltKafkaTemplate) {
        Counter dltPublishedCounter = Counter.builder("account.kafka.dlt.published")
     ...
     */
    @Bean
    public DefaultErrorHandler accountKafkaErrorHandler(DeadLetterPublishingRecoverer accountDeadLetterRecoverer) {
        FixedBackOff backOff = new FixedBackOff(RETRY_INTERVAL_MS, MAX_RETRIES);
        DefaultErrorHandler handler = new DefaultErrorHandler(accountDeadLetterRecoverer, backOff);
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> kafkaListenerContainerFactory(
            ...
            DefaultErrorHandler accountKafkaErrorHandler) {
        ConcurrentKafkaListenerContainerFactory<String, String> factory =
                new ConcurrentKafkaListenerContainerFactory<>();
        factory.setConsumerFactory(accountConsumerFactory);
        factory.setCommonErrorHandler(accountKafkaErrorHandler);
