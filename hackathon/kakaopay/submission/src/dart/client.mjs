...
const DART_OK_STATUS = '000';
const DART_NO_DATA_STATUS = '013';
const ZIP_LOCAL_FILE_HEADER = 0x04034b50;
const RESPONSE_PREVIEW_BYTES = 300;
const REQUEST_TIMEOUT_MS = 20_000;

export const API_KEY = findEnvKey('DART_API_KEY', MODULE_DIR);
...
async function getRaw(path, params = {}) {
  ...
  const res = await fetch(`${BASE}/${path}?${q}`, { signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) });
...
export async function getJson(path, params = {}) {
  ...
  if (body.status && body.status !== DART_OK_STATUS && body.status !== DART_NO_DATA_STATUS) {
...
export async function getBinary(path, params = {}) {
  ...
  if (buf.length < 4 || buf.readUInt32LE(0) !== ZIP_LOCAL_FILE_HEADER) {
    const head = buf.toString('utf8', 0, Math.min(buf.length, RESPONSE_PREVIEW_BYTES));
