/**
 * notification-service (Kotlin, 8130) — 알림 푸시 SSE 클라이언트.
 *
 * 경로 계약: 프론트는 항상 `/api/notifications/stream` 으로 구독한다.
 *  - dev: vite proxy 가 `/api` 를 벗겨 8130 `/notifications/stream` 으로 직결
...
/** 서버 푸시 페이로드 (notification-service StreamEventDto 와 계약) */
