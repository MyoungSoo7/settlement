...
/**
 * App.tsx 의 라우트 가드 4종 중 `AdminManagerRoute` 만 테스트가 있었다
 * (AppWorkforceAccess.test.tsx). 나머지 셋 — ProtectedRoute · PrintRoute ·
 * AdminOnlyRoute — 은 한 줄도 지나가지 않았다.
 *
 * 이 셋은 서로 *다른 곳으로* 튕긴다. 미인증일 때 ProtectedRoute 는 사용자 로그인,
...
vi.mock('@/pages/print/SettlementPrintPage', () => ({
  default: () => <div>정산 인쇄 문서</div>,
}));
...
describe('라우트 가드', () => {
  ...
  describe('PrintRoute (/print/settlement/:id — 인쇄 전용, ADMIN·MANAGER)', () => {
    it('미인증이면 관리자 로그인으로 보낸다', async () => {
      visit('/print/settlement/1');

      expect(await screen.findByText('관리자 시스템')).toBeInTheDocument();
      expect(screen.queryByText('정산 인쇄 문서')).not.toBeInTheDocument();
    });
    ...
    it.each(['ADMIN', 'MANAGER'])('%s 에게 문서를 열어 준다', async (role) => {
      signIn(role);

      visit('/print/settlement/1');

      expect(await screen.findByText('정산 인쇄 문서')).toBeInTheDocument();
    });
  });
