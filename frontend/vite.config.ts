...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      //
      // 실측 91.17%(2026-08-14) — 임계치 90 대비 약 35줄 여유다. 임계치를 실측치까지 바짝
      // 올리지 않는 이유가 이 여유다: 기능 한 줄 추가에 게이트가 우는 상태면 다들 임계치를
      // 내리는 쪽으로 대응하게 되고, 그러면 게이트가 숫자만 남는다. 여유가 소진되면
      // 임계치를 내리지 말고 새 코드에 테스트를 붙일 것.
      //
      // lines 만 거는 이유: statements(88.8%)·branches(83.0%)는 아직 90 에 못 미친다. 지금 함께
