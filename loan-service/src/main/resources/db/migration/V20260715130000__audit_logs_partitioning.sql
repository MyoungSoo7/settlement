...
-- 기준 스키마: 기존 V5__audit_logs.sql (무접두, default_schema=opslab). 인덱스 3종은 이번에 표준으로 신규 추가.

-- 1) 기존 테이블·PK 리네임 (이름 충돌 회피)
ALTER TABLE audit_logs RENAME TO audit_logs_old;
ALTER TABLE audit_logs_old RENAME CONSTRAINT audit_logs_pkey TO audit_logs_old_pkey;

-- 2) 파티션드 부모 — 컬럼 구성 V5 와 동일, PK 만 (id, created_at). 기존 시퀀스 재사용으로 연속성 보존.
...
-- 5) 인덱스 표준 3종 신규 생성 (기존 V5 엔 없었음 — 전 서비스 표준화)
