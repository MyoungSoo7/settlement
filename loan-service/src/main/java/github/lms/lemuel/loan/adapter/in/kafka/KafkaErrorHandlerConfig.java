...
/**
 * loan 서비스 Kafka 컨슈머 에러 핸들링 설정(settlement/investment {@code KafkaErrorHandlerConfig} 동형 배선).
 *
 * <p>배경(감사 지적): 기존엔 Spring Kafka 기본 에러핸들러({@code FixedBackOff(0, 9)})만 있어
 * 재시도 소진 후 메시지를 조용히 skip = 사실상 유실했다. settlement.created/confirmed 는 각각 담보
 * 프로젝션·상환 saga 의 유일한 트리거이므로 유실 시 여신 판단·상환 회계가 영구 결손된다.
 *
 * <p>핵심 동작:
 * <ol>
 ...
 * 진단 헤더를 함께 부여해 사후 추적·멱등 replay 를 보장한다. 메트릭 {@code loan.kafka.dlt.published}
...
public class KafkaErrorHandlerConfig {
    ...
    public ConsumerFactory<String, String> loanConsumerFactory(
            @Value("${spring.kafka.consumer.group-id:lemuel-loan}") String groupId) {
        Map<String, Object> props = new HashMap<>();
     ...
     */
    @Bean
    public DeadLetterPublishingRecoverer loanDeadLetterRecoverer(KafkaTemplate<String, String> dltKafkaTemplate) {
        Counter dltPublishedCounter = Counter.builder("loan.kafka.dlt.published")
     ...
     */
    @Bean
    public DefaultErrorHandler loanKafkaErrorHandler(DeadLetterPublishingRecoverer loanDeadLetterRecoverer) {
        FixedBackOff backOff = new FixedBackOff(RETRY_INTERVAL_MS, MAX_RETRIES);
        DefaultErrorHandler handler = new DefaultErrorHandler(loanDeadLetterRecoverer, backOff);
        ...
        Counter retryCounter = Counter.builder("loan.kafka.retry")
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> kafkaListenerContainerFactory(
            ConsumerFactory<String, String> loanConsumerFactory,
