...
/**
 * loan 서비스 Kafka DLT 배선의 단위 검증 — 재시도 소진 시 유실 대신 DLT 로 보내는 핸들러와
...
class KafkaErrorHandlerConfigTest {
    ...
    void buildsErrorHandlerAndRecoverer() {
        ...
        DeadLetterPublishingRecoverer recoverer = config.loanDeadLetterRecoverer(template);
        DefaultErrorHandler handler = config.loanKafkaErrorHandler(recoverer);

        assertThat(handler).isInstanceOf(DefaultErrorHandler.class);
        assertThat(recoverer).isNotNull();
        assertThat(registry.find("loan.kafka.dlt.published").counter()).isNotNull();
    }

    @Test
    @DisplayName("리스너 팩토리는 수동 즉시 ack + 지정 동시성으로 조립된다")
    void buildsListenerContainerFactory() {
        ConsumerFactory<String, String> cf = config.loanConsumerFactory("lemuel-loan");
        DefaultErrorHandler handler = config.loanKafkaErrorHandler(
                config.loanDeadLetterRecoverer(config.dltKafkaTemplate(config.dltProducerFactory())));
