...
| 모드 | 입력 | 명령 | 판정 축 |
|---|---|---|---|
| **기본 (API-only)** | 기업명 또는 corp_code (코스피·코스닥 상장사) | `node src/bin/diagnose-company.mjs --company 삼성전자` | 외부 신호 E1~E5 — 수익-채권 괴리 · 재고 적체 · 차입 확대·이자 부담 · 유동성 하락 · 공시 행간 (+ ECOS 기준금리 컨텍스트) |
...
| API-only 기업 진단 (외부 신호 E1~E5) | `src/bin/diagnose-company.mjs` (+ `src/common/dart-signals.mjs`) |
...
| API-only 진단 | 내부 CSV 없이 외부 신호 E1~E5 가 3개년 공시·공시 목록에서 파생되는가 — 삼성전자 라이브(E5 만 PRESENT, 건전 지표는 근거 수치와 함께 absent), 스트레스/건전 픽스처 회귀(`dart-signals.test.mjs`) |
