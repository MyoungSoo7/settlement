...
function findEnvKey() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (existsSync(p)) {
      const m = readFileSync(p, 'utf8').match(/^ECOS_API_KEY=([^\r\n#]+)/m);
...
export async function statisticSearch({ statCode, itemCode, cycle, start, end }) {
  requireKey();
  const url = `${BASE}/StatisticSearch/${API_KEY}/json/kr/1/10000/${statCode}/${cycle}/${start}/${end}/${itemCode}`;
  const body = await getJson(url);

  const result = body?.RESULT ?? body?.StatisticSearch?.RESULT;
  if (result?.CODE) {
    if (result.CODE === 'INFO-200') return [];   // 조회 데이터 없음 — 에러 아님
