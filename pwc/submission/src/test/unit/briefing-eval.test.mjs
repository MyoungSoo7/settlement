...
test('음성 모드 — 미발화 명시 절의 카테고리 정성 요약은 오탐이 아니다', () => {
  // 신성통상 라이브 회귀: 미발화 신호를 "임계값 미달" 로 투명하게 요약하고
  // 같은 절에 "가능성" 같은 단어가 있어도 리스크 주장으로 보지 않는다.
  const briefing = `# 브리핑
## 1. 종합 판단
이번 진단에서 발화(PRESENT)한 항목은 없습니다. 확인된 경영리스크는 포착되지 않았습니다.
## 2. 신호 평가 — 전 항목 미발화
매출채권 괴리와 재고 적체 모두 임계값 미달로 미발화했습니다. 리스크 승격 오해 가능성을 피하기 위해 수치는 인용하지 않습니다.
확신도: 확인 필요 — 다음 분기 재평가.
권고 조치: 분기 재실행.
`;
  const signals = [
    { id: 'E1', name: '수익-채권 괴리', present: false, markers: [/46\.9/], categoryPattern: /매출채권|수익.채권/ },
    { id: 'E2', name: '재고 적체', present: false, markers: [/160/], categoryPattern: /재고/ },
  ];
  const r = evaluateBriefing(briefing, { signals });
  assert.equal(r.negative, true);                 // PRESENT 0건 → 자동 음성 채점
  assert.deepEqual(r.falseSignals, []);           // 미발화 명시 절은 주장 아님
  assert.equal(r.cleanAck, true);                 // "포착되지 않았습니다" 도 승인으로 인정
  assert.equal(r.pass, true);
});

test('마커 동률 — 요약과 리스크 절이 같은 수치를 인용하면 확신도 태그 있는 절을 채점 (LG엔솔 라이브 회귀)', () => {
  const briefing = `# 브리핑
## 요약
유동비율이 109.7%까지 하락했습니다. 유동성 점검이 필요합니다.
## 리스크 1 — 유동성 하락
결론: 유동비율 109.7%로 내려왔습니다. 유동성 버퍼가 얇아졌습니다.
근거: DART 연결재무제표 유동비율 추이.
확신도: 확인됨 — 공시 수치 기준.
판별 테스트: 만기 스케줄 대조.
권고 조치: 롤링 유동성 계획 보고.
`;
  const signals = [{
    id: 'E4', name: '유동성 하락', present: true,
    markers: [/(?<!\d)109[.,]7(?!\d)/, /유동비율|유동성/], categoryPattern: /유동/,
  }];
  const r = evaluateBriefing(briefing, { signals });
  assert.equal(r.recall, 1);
  assert.match(r.signals[0].section, /리스크 1/);   // 요약(선행 동률)이 아니라 확신도 있는 절
  assert.deepEqual(r.calibrationMisses, []);
  assert.deepEqual(r.discriminationMisses, []);
  assert.equal(r.pass, true);
});

test('음성 모드 — 이상 없음 승인의 어순 변형 인정 (삼성전기 라이브 회귀)', () => {
  const signals = [{ id: 'E1', name: 'x', present: false, markers: [/999/], categoryPattern: /zzz/ }];
  for (const ack of [
    '경영리스크는 이번 창에서 포착되지 않았습니다.',
    '본 브리핑에는 확정 리스크 항목이 없으며 정성 요약만 담습니다.',
    '서명이 필요한 리스크 신호는 없습니다.',
  ]) {
    const r = evaluateBriefing(`# 브리핑\n## 종합 판정\n${ack}\n확신도: 확인 필요.\n권고 조치: 분기 재실행.\n`, { signals });
    assert.equal(r.cleanAck, true, ack);
    assert.equal(r.pass, true, ack);
  }
});

test('음성 모드 — 미발화 신호의 근거 수치를 옮겨 적으면 여전히 오탐 (마커 경로 유지)', () => {
  const briefing = `# 브리핑
## 1. 종합 판단
확인된 경영리스크는 포착되지 않았습니다.
## 2. 신호 평가 — 전 항목 미발화
E1 은 미발화지만 매출채권 46.9% 증가, 영업현금흐름 -310 이었습니다.
`;
  const signals = [
    { id: 'E1', name: '수익-채권 괴리', present: false, markers: [/46\.9/, /-310/], categoryPattern: /매출채권/ },
  ];
  const r = evaluateBriefing(briefing, { signals });
  assert.deepEqual(r.falseSignals, ['E1']);       // 수치 복붙은 미발화 서술과 무관하게 포착
  assert.equal(r.pass, false);
});
