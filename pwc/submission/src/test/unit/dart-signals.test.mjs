...
const STRESSED_BODY = {
  ...
  list: [
    ...
    row('IS', 'ifrs-full_ProfitLoss', '당기순이익', [70, 90, 100]),                    // E8: NI 100 vs OCF 40
    row('BS', 'ifrs-full_Assets', '자산총계', [400, 420, 450]),                        // 발생액 (100−40)/450=13.3%
  ],
};
...
const HEALTHY_BODY = {
  ...
  list: [
    ...
    row('IS', 'ifrs-full_ProfitLoss', '당기순이익', [70, 80, 90]),                     // E8: 발생액 음수 (OCF>NI)
    row('BS', 'ifrs-full_Assets', '자산총계', [1000, 1050, 1100]),
  ],
};
...
test('dart-signals — 스트레스 공시: E1~E5 전부 PRESENT + 근거 수치', () => {
  ...
  assert.deepEqual(signals.filter((s) => s.present).map((s) => s.id), ['E1', 'E2', 'E3', 'E4', 'E5', 'E8']);
...
test('dart-signals — 계정 결측이면 해당 신호 evaluable=false (지어내지 않음)', () => {
  ...
  for (const id of ['E1', 'E2', 'E3', 'E4', 'E5', 'E8']) {
...
// ── E8 발생액 품질 ───────────────────────────────────────────
test('dart-signals — E8: 발생액 비율 13.3% ≥ 10% 상한 → PRESENT + 계산값 마커', () => {
  const series = extractFullSeries(STRESSED_BODY);
  const signals = deriveExternalSignals({ series, disclosures: STRESSED_DISCLOSURES, days: 90 });
  const e8 = signals.find((s) => s.id === 'E8');
  assert.equal(e8.present, true);
  assert.equal(e8.evidence.accrualRatioPct, 13.3);
  assert.equal(e8.evidence.ocfToNetIncome, 0.4);
  assert.equal(e8.evidence.persistentGapTwoYears, false); // 전기 OCF/NI 0.67 ≥ 0.5 — 비율 조건 단독 발화
  const sample = '순이익 100 대비 영업현금흐름 40 — 총자산 대비 발생액 비율 13.3%로 현금 뒷받침 없는 이익.';
  assert.ok(e8.markers.every((m) => m.test(sample)), '마커가 서술 문장에서 재현되어야 함');
  assert.ok(e8.categoryPattern.test('발생액 품질 리스크'));
});

test('dart-signals — E8: 지속 괴리(2년 연속 OCF/NI < 0.5)만으로도 PRESENT (비율 마커 없이 성립)', () => {
  const body = {
    status: '000',
    list: [
      row('IS', 'ifrs-full_ProfitLoss', '당기순이익', [80, 100, 100]),
      row('CF', 'ifrs-full_CashFlowsFromUsedInOperatingActivities', '영업활동현금흐름', [70, 40, 30]),
      row('BS', 'ifrs-full_Assets', '자산총계', [2000, 2000, 2000]), // 발생액 비율 3.5% < 10
    ],
  };
  const e8 = deriveExternalSignals({ series: extractFullSeries(body), disclosures: null }).find((s) => s.id === 'E8');
  assert.equal(e8.present, true);
  assert.equal(e8.evidence.persistentGapTwoYears, true);
  assert.ok(e8.markers.length > 0);
});
...
`;
  ...
  assert.equal(r.total, 6); // E1~E5 + E8 (스트레스 픽스처 전부 PRESENT)
...
test('diagnose-company CLI — 기본 모드(--json): 외부 신호 6건 + 거시 생략 + 내부 null', () => {
  ...
  try {
    ...
    assert.equal(out.externalPresent, 6);
