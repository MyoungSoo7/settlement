...
test('docx — briefingToDocx: OPC 구조 + 한글 무손실 + 한글 폰트 + 서식 요소', () => {
  ...
  // 러닝 헤더: ASCII 문구(헤드리스 LO 한글 glyph fallback 회피), 표지는 titlePg 로 헤더/푸터 제외
  assert.match(entries['word/header1.xml'], /CEO Risk Briefing/);
  assert.match(entries['word/header1.xml'], /Confidential/);
  ...
  // 푸터: ASCII 면책 + PAGE 필드
  assert.match(entries['word/footer1.xml'], /Not investment advice/);
...
test('docx — snapshot PNG: DOCX media part + drawing relationship 포함', () => {
  const dir = mkdtempSync(join(tmpdir(), 'tca-docx-img-'));
  try {
    const png = join(dir, 'snapshot.png');
    writeFileSync(png, Buffer.from(
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAFgwJ/l6lk7wAAAABJRU5ErkJggg==',
      'base64',
    ));
    const entries = readZip(briefingToDocx(SAMPLE_MD, { snapshotImagePath: png }));
    assert.ok(entries['word/media/executive-summary.png'], 'PNG part missing');
    assert.match(entries['[Content_Types].xml'], /ContentType="image\/png"/);
    assert.match(entries['word/_rels/document.xml.rels'], /rIdSnapshot/);
    assert.match(entries['word/document.xml'], /Executive Snapshot/);
    assert.match(entries['word/document.xml'], /r:embed="rIdSnapshot"/);
  } finally {
...
test('docx — briefingToDocx: h1/메타 없는 최소 마크다운도 렌더 (제목 폴백)', () => {
  ...
  assert.ok(!/Confidential/.test(entries['word/header1.xml']));
});
...
test('docx — extractRiskSummary: 콜론 없는 굵은 라벨 + 번호 목록 권고 조치 수집 (네이버 라이브 회귀)', () => {
  const md = `# T
## 리스크 1 — 공시 행간 (E5)
**결론**: 정정이 반복됩니다.
**확신도**: 확인됨 — 공시 목록 기준.
**권고 조치**
- IR 임원에게 판별 테스트 결과를 2주 내 보고받으십시오.

## 리스크 2 — 유동성 (E4)
확신도: 가설.
권고 조치:
1. CFO 에게 12개월 롤링 유동성 계획을 보고받으십시오.
2. 두번째 조치.
`;
  const risks = extractRiskSummary(parseBlocks(md));
  assert.equal(risks.length, 2);
  assert.match(risks[0].action, /^IR 임원에게/);           // 콜론 없는 **라벨** 다음 불릿
  assert.equal(risks[0].confidence, '확인됨');
  assert.match(risks[1].action, /^CFO 에게/);              // 번호 목록 첫 항목 (번호 제거)
});
