...
const input = argv.find((a) => !a.startsWith('--') && a !== flag('--out') && a !== flag('--title'));

if (!input || argv.includes('--help') || argv.includes('-h')) {
  console.log('사용법: node src/bin/render-briefing-docx.mjs <briefing.md> [--out <briefing.docx>] [--title <제목>]');
  process.exit(input ? 0 : 1);
}

const outPath = flag('--out') ?? input.replace(/\.md$/i, '') + '.docx';
const markdown = readFileSync(input, 'utf8');
const buffer = briefingToDocx(markdown, { title: flag('--title') });
