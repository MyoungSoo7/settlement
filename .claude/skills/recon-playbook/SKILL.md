...
| D일 결제>0·정산=0 인데 D+N일에 반대 부호 불일치 | **날짜 기준 의미 차이** — 정산 축이 지급예정일(`settlement_date`, T+N 영업일)로 집계됨 | settlements 를 payment_id 로 찾아 `created_at` vs `settlement_date` 비교. 대사는 생성일(created_at) 기준이어야 함 (2026-07-06 실사례 — DLT/lag 전부 정상인데 불일치였음) |
| 환불 축만 불일치 (refundDiscrepancy>0) | 역정산 조정 미생성 / 환불 이벤트 타이밍 (자정 경계) | settlement_adjustments 의 refund_id 연결분, processed_events 에서 PaymentRefunded 를 소비한 그룹 목록 (뷰 그룹만 있으면 역정산 컨슈머 미결선) |
...
> DLT 비어 있음 + 컨슈머 lag 0 + processed_events 기록 존재 = "유실"이 아니다.
> 그때는 **집계 쿼리의 날짜/상태 기준**(어느 컬럼으로 자르는지)을 의심하라.

## 대사 축 설계 규칙 (2026-07 재정립)

일일 대사는 **캡처일 하나로 키를 맞춘** 두 축으로 대조한다:

- **캡처 축**: order 캡처 gross(CAPTURED+REFUNDED) == settlement 정산 gross(`payment_amount`, 생성일 기준).
- **환불 축**: order 캡처분 반영 환불(`refunded_amount`, 캡처일 기준) == settlement 정산 `refunded_amount`(생성일 기준).

안정 컬럼만 쓴다: `payment_amount`·`refunded_amount` 는 환불로 소급 변동하지 않는다.
**`net_amount` 를 대사 기준에 쓰지 마라** — 역정산이 실시간으로 깎아 "생성일 net+commission == gross" 가
환불 후 깨진다(2026-07 실수). 지급예정일(`settlement_date`)·환불 완료일로 키를 잡아도 T+N·백필에 흔들린다.
불변식: `payment_amount = net_amount + commission + refunded_amount` (항상 성립).
