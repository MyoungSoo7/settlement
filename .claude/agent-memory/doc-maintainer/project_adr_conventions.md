...
touch unrelated docs when the task is scoped to ADR-only work — confirm scope before touching
adjacent docs.

- **커밋 규칙**: ADR 추가·삭제 시 문서에 ADR 개수를 박제하지 않는다(2026-08-07 STATUS.md 폐지 — 수치 정본 문서를 두면 병행 세션이 갱신을 놓쳐 CI 가 중간 커밋을 차단했다).
