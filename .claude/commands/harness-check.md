...
1. `node scripts/harness/harness-audit.mjs` 를 실행하고 4개 섹션 결과를 그대로 보고하라:
   라우팅 맵 dangling · settings.json 가드 훅 경로 실존 · 모듈 로스터 · 인벤토리.
...
4. `node scripts/harness/telemetry-report.mjs` 를 실행해 텔레메트리 요약(가드 규칙별 발화 횟수·0회
   규칙·스킬 사용률·라우터 제안 대비 미로드)을 함께 보고하라. "no data yet" 이면 그대로 보고
   (텔레메트리는 관측이지 게이트가 아님 — 이 단계의 어떤 결과도 진단을 실패시키지 않는다).
