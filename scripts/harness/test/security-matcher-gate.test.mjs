...
/**
 * 서비스별로 **실제 적용되는** SecurityConfig 를 고른다.
 *
 * <p>자체 SecurityConfig 가 있으면 그것이 유일한 필터체인이다(두 벌이면 빈 충돌이라 공존하지 않는다).
 * 없으면 shared-common 의 것을 쓴다. 이 구분을 빼먹으면 board·education 처럼 **자체 설정으로
 * 멀쩡히 ADMIN 보호 중인 19개 엔드포인트가 통째로 미보호로 뜬다**(실제로 그렇게 떴다).
 */
export function securityConfigFor(root, service) {
  const own = walk(join(root, service, 'src', 'main', 'java'))
    .find((f) => basename(f) === 'SecurityConfig.java');
  if (own) return { path: own, scope: 'own' };
  const shared = join(root, 'shared-common/src/main/java/github/lms/lemuel/common/config/jwt/SecurityConfig.java');
  return existsSync(shared) ? { path: shared, scope: 'shared-common' } : null;
}
...
export function findUnprotected(root, services = JAVA_SERVICES) {
  ...
  for (const service of services) {
    ...
    const matchers = config ? parseMatchers(readFileSync(config.path, 'utf8')) : [];
...
describe('민감 경로 인가 출처 게이트', () => {
  ...
  test('민감 경로를 실제로 훑었다 — 0건 통과가 "못 찾아서"가 아님을 증명', () => {
    ...
    assert.ok(sensitive >= 150, `민감 엔드포인트 수집이 비정상적으로 적다(${sensitive}건) — 추출기가 깨졌을 수 있다`);
  });
...
describe('검출기 자기검증 — 실제로 밟은 버그들', () => {
  ...
  test('자체 SecurityConfig 를 가진 서비스는 그것으로 판정한다', () => {
    // board·education 은 자체 설정으로 /admin/** 을 ADMIN 으로 막는다.
    // shared-common 설정만 보면 이 19건이 통째로 미보호로 뜬다(실제로 그렇게 떴다).
    for (const service of ['board-service', 'education-service']) {
      const config = securityConfigFor(repoRoot, service);
      assert.equal(config.scope, 'own', `${service} 는 자체 SecurityConfig 를 쓴다`);
    }
  ...
  test('위성 서비스는 AdminApiKeyFilter 배선을 인정받는다', () => {
    assert.equal(hasAdminApiKeyFilter(repoRoot, 'market-service'), true);
