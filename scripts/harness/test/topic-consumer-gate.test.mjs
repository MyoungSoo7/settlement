...
const PUBLISH_ONLY = new Map([
  // card — 청구 사이클 3단계에서 소비 예정 (SPEC.md §5)
  ['lemuel.card.account_opened', 'SPEC §5 발행 전용 — 청구 사이클 3단계에서 소비 예정'],
  ['lemuel.card.account_status_changed', 'SPEC §5 발행 전용 — 청구 사이클 3단계에서 소비 예정'],
  ['lemuel.card.limit_changed', 'SPEC §5 발행 전용 — 청구 사이클 3단계에서 소비 예정'],
  ['lemuel.card.issued', 'SPEC §5 발행 전용 — 청구 사이클 3단계에서 소비 예정'],
  ['lemuel.card.status_changed', 'SPEC §5 발행 전용 — 청구 사이클 3단계에서 소비 예정'],
  ['lemuel.card.authorized', '발행 전용 — Phase 2 승인 홀드는 card 내부에서 생성된다(발행은 관측용)'],
  ['lemuel.card.statement_paid', '발행 전용 — 명세서 전액 납부 통지(ADR 0022), 소비처 미배선'],
  // insurance 9종 (SPEC.md §5) — 계약 스키마 편입 완료(2026-08-22), 소비처만 아직 없다
  ['lemuel.insurance.policy_issued', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.policy_status_changed', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.banca_rule_violated', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.commission_confirmed', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.commission_paid', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.commission_clawback_triggered', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.general_payout_requested', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.general_payout_paid', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  ['lemuel.insurance.commission_monthly_closed', 'SPEC §5 발행 전용 — 소비처 미배선(계약 스키마·프로듀서 테스트는 편입됨, ADR 0024)'],
  // deposit 5종 (SPEC.md §5 · deposit PRD G-2 — "격차 아님, 정책대로")
  ['lemuel.deposit.balance_changed', 'SPEC §5 발행 전용 — 소비처 생길 때 계약 편입'],
  ['lemuel.deposit.hold_placed', 'SPEC §5 발행 전용 — 소비처 생길 때 계약 편입'],
  ['lemuel.deposit.hold_released', 'SPEC §5 발행 전용 — 소비처 생길 때 계약 편입'],
  ['lemuel.deposit.offset_applied', 'SPEC §5 발행 전용 — 소비처 생길 때 계약 편입'],
...
describe('미소비 토픽 (SPEC.md §5 발행 전용 정책)', () => {
  ...
  test('스캔이 실제로 리스너에 도달했다', () => {
    assert.ok(expressions >= 60, `리스너 표현식 수집이 적다(${expressions}) — 추출기가 깨졌을 수 있다`);
    assert.ok(consumed.size >= 40, `소비 토픽 수집이 적다(${consumed.size})`);
    // 확실히 소비되는 토픽이 안 잡히면 나머지 판정을 신뢰할 수 없다.
    for (const known of ['lemuel.settlement.created', 'lemuel.order.created', 'lemuel.card.captured']) {
