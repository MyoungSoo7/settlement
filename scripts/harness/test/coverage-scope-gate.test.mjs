...
describe('커버리지 측정 범위 게이트', () => {
  test('모듈 로스터를 settings.gradle.kts 에서 읽어온다(검사가 실제로 돌았음을 증명)', () => {
    assert.ok(modules.length >= 19, `모듈 로스터를 못 읽었다: ${JSON.stringify(modules)}`);
    assert.ok(modules.includes('deposit-service'));
    assert.ok(modules.includes('board-service'));
    assert.ok(modules.includes('education-service'));
  });
