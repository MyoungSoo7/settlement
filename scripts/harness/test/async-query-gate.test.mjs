...
const STATIC_QUERIES = new Map([
  ...
  ["ChargebackConsolePage.test.tsx :: screen.getByRole('button', { name: '수동 등록' })", '헤더 버튼 — 마운트부터 있다'],
  ["ChargebackConsolePage.test.tsx :: screen.getByRole('button', { name: '수락(셀러 부담)' })", '상태 필터 탭 — 마운트부터 있다'],

  // LedgerConsolePage.tsx 의 단건 추적 폼. 조회 결과와 무관하게 조건 없이 렌더된다.
  ["LedgerConsolePage.test.tsx :: screen.getByPlaceholderText('예: 1024')", '단건 추적 입력 — 조건 없이 렌더'],
  ["LedgerConsolePage.test.tsx :: screen.getByRole('button', { name: '추적' })", '단건 추적 버튼 — 조건 없이 렌더'],
