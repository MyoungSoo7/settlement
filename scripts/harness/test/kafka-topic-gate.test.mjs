...
describe('Kafka 토픽 카탈로그 (ADR 0035)', () => {
  ...
  test('스캔이 실제로 서비스 설정에 도달했다', () => {
    assert.ok(configs.length >= 10, `application.yml 스캔 결과가 적다(${configs.length})`);
    assert.ok(referenced.size >= 30, `참조 토픽 수집이 적다(${referenced.size}) — 추출기가 깨졌을 수 있다`);
  });
