...
const ALLOWANCE =
  /harness-guard:\s*allow\s+reason="([^"]*)"\s+issue="([^"]*)"\s+owner="([^"]*)"\s+expires="([^"]*)"\s*$/i;
const ISSUE =
  /^(ISSUE-\d+|https:\/\/github\.com\/[^/\s]+\/[^/\s]+\/issues\/\d+)$/;
const OWNER = /^team-[a-z0-9]+(?:-[a-z0-9]+)*$/;

const POLICY_ROOT =
  /(?:^|\/)((?:hackathon|pwc|settlement-service|account-service|market-service)\/.*)$/;
...
const MONEY_SCOPE =
  /(settlement|ledger|payout|chargeback|loan|payment|investment|account|pgreconciliation|recon|insurance)/i;
...
const isDomainMain = (f) =>
  JAVA_KT.test(f) &&
  /\/src\/main\/java\/.+\/domain\//.test(f) &&
  !/common\/audit\//.test(f);
// generic 예외 금지는 캠페인이 청정화를 완료한 5개 금융 서비스에 한정(위성 서비스는 oo-score 스킬로 채점).
const CAMPAIGN_SERVICES =
  /(settlement|order|loan|investment|account|insurance)-service\//;
...
export const RULES = [
  {
    ...
    when: (f) =>
      JAVA_KT.test(f) && MONEY_SCOPE.test(f) && isCore(f) && isProd(f),
    test: (line) =>
  ...
  },
  {
    ...
    when: (f) =>
      SQL.test(f) || (JAVA_KT.test(f) && MONEY_SCOPE.test(f) && isProd(f)),
    test: (line) =>
      /\b(UPDATE|DELETE\s+FROM)\s+(settlements|ledger_entries|payouts|account_entries)\b/i.test(
        line,
      ),
  ...
  },
  {
    ...
    test: (line) => {
      ...
      const m =
        /^\s*import\s+(?:static\s+)?github\.lms\.lemuel\.([a-z0-9_]+)\b/.exec(
          line,
        );
  ...
  },
  {
    ...
    test: (line) =>
      /\bkafkaTemplate\.\s*send\s*\(/.test(line) ||
      /SaveOutboxEventPort/.test(line),
  ...
  },
  {
    ...
    test: (line) =>
      /\b(PER|PBR|priceEarnings|priceToBook)\b/.test(line) && /=/.test(line),
...
export function parseAllowance(line, { now = new Date() } = {}) {
  ...
  if (!reason.trim() || !ISSUE.test(issue) || !OWNER.test(owner) || !dateMatch)
    return null;
...
export function scanText(f, content, { now = new Date() } = {}) {
  ...
  for (const rule of RULES) {
    ...
    lines.forEach((line, i) => {
      ...
      if (rule.test && rule.test(line))
        violations.push({ file: f, line: i + 1, id: rule.id, msg: rule.msg });
    });
    ...
    const pattern = new RegExp(
      rule.fileTest.source,
      rule.fileTest.flags.includes("g")
        ? rule.fileTest.flags
        : `${rule.fileTest.flags}g`,
    );
    for (const match of String(content).matchAll(pattern)) {
      const lineNumber = String(content)
        .slice(0, match.index)
        .split(/\r?\n/).length;
...
export async function normalizeRepoPath(repoRoot, requestedPath) {
  ...
  if (!isContained(root, requested))
    throw new Error("path is outside repository or is repository root");
  const ancestor = await nearestExistingAncestor(requested);
  const actualAncestor = await realpath(ancestor);
  if (actualAncestor !== root && !isContained(root, actualAncestor))
    throw new Error("path escapes repository through a link");
  return requested;
}
...
function replaceExactlyOnce(content, oldString, newString) {
  ...
  if (first < 0 || content.indexOf(oldString, first + oldString.length) >= 0)
...
export async function reconstructPendingContent(event, { repoRoot }) {
  ...
  if ("new_path" in input || "destination" in input)
    throw new Error("rename is unsupported");
  ...
  if (tool === "Edit")
    return replaceExactlyOnce(
      await readUtf8Strict(filePath),
      input.old_string,
      input.new_string,
    );
  if (tool === "MultiEdit") {
    if (!Array.isArray(input.edits) || input.edits.length === 0)
      throw new Error("MultiEdit edits are required");
    let content = await readUtf8Strict(filePath);
    for (const edit of input.edits)
      content = replaceExactlyOnce(content, edit?.old_string, edit?.new_string);
    return content;
  }
...
export function dodNudgeMessage(files) {
  ...
  const money = normalized.filter(
    (f) => JAVA_KT.test(f) && MONEY_SCOPE.test(f) && isCore(f) && isProd(f),
  );
  if (money.length === 0) return null;
  if (normalized.some((f) => JAVA_KT.test(f) && /\/src\/test\//.test(f)))
    return null;
...
export function discoverStagedFiles(repoRoot) {
  const output = execFileSync(
    "git",
    ["diff", "--cached", "--name-only", "-z", "-M", "--diff-filter=ACMR"],
    { cwd: repoRoot },
  );
...
function emitReport(violations, io = {}) {
  ...
  if (violations.length === 0) {
    stdout("harness guard: clean");
    return 0;
  }
  stderr(`harness guard: ${violations.length} blocking violation(s)`);
  for (const violation of violations)
    stderr(
      `[${violation.id}] ${violation.file}${violation.line ? `:${violation.line}` : ""} ${violation.msg}`,
    );
  return 1;
}
...
export async function runGuardCli(args, io = {}) {
  ...
  if (modes.length !== 1) {
    stderr("exactly one guard mode is required");
    return 2;
  }
  const mode = modes[0];
  if (mode === "--self-test") {
    ...
    return (
      spawnSync(
        ...
        [
          "--test",
          fileURLToPath(new URL("./test/guard.test.mjs", import.meta.url)),
        ],
  ...
  if (mode === "--hook") {
    ...
    try {
      ...
      stderr(`guard hook rejected input: ${error.message}`);
      return 2;
    }
  }
  try {
    ...
    if (mode === "--staged") {
      ...
      files = (await readUtf8Strict(listPath))
        .split(/\r?\n/)
        .map((item) => item.trim())
        .filter(Boolean);
    } else {
    ...
    for (const file of files)
      violations.push(...(await scanRepoFile(repoRoot, file)));
    ...
    if (mode === "--staged") {
      ...
      if (nudge) {
        ...
        await appendJsonl(repoRoot, "dod-nudges.jsonl", [
          { ts: new Date().toISOString(), staged: files.length },
        ]);
      }
    }
    return emitReport(violations, io);
  } catch (error) {
    stderr(`guard input failed: ${error.message}`);
    return 1;
  }
}
