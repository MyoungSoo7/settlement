-- ============================================================
-- V1 : board_definitions — 메타 주도 게시판 플랫폼의 뿌리 테이블
--
...
ALTER TABLE board_definitions DROP CONSTRAINT IF EXISTS ck_board_skin;
ALTER TABLE board_definitions ADD CONSTRAINT ck_board_skin
    CHECK (skin IN ('LIST', 'GALLERY', 'FAQ', 'QNA'));

ALTER TABLE board_definitions DROP CONSTRAINT IF EXISTS ck_board_content_format;
ALTER TABLE board_definitions ADD CONSTRAINT ck_board_content_format
    CHECK (content_format IN ('TEXT', 'MARKDOWN', 'HTML'));

-- 이용 목록 조회는 항상 "활성만, 이름순"이다.
CREATE INDEX IF NOT EXISTS idx_board_definitions_active ON board_definitions(active, name);
