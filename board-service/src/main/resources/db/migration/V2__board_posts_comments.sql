-- ============================================================
-- V2 : board_posts / board_comments — 게시글과 댓글
--
...
CREATE TABLE IF NOT EXISTS board_posts (
    ...
    board_id       BIGINT        NOT NULL REFERENCES board_definitions(id),
...
ALTER TABLE board_posts DROP CONSTRAINT IF EXISTS ck_board_post_status;
ALTER TABLE board_posts ADD CONSTRAINT ck_board_post_status
    CHECK (status IN ('PUBLISHED', 'HIDDEN', 'DELETED'));

ALTER TABLE board_posts DROP CONSTRAINT IF EXISTS ck_board_post_content_format;
ALTER TABLE board_posts ADD CONSTRAINT ck_board_post_content_format
...
CREATE INDEX IF NOT EXISTS idx_board_posts_list
    ON board_posts(board_id, status, pinned DESC, created_at DESC);

-- 내 글 찾기(비밀글 소유 판정)는 작성자 기준 조회다.
CREATE INDEX IF NOT EXISTS idx_board_posts_author ON board_posts(board_id, author_id);
...
CREATE TABLE IF NOT EXISTS board_comments (
    ...
    board_id    BIGINT        NOT NULL REFERENCES board_definitions(id),
    parent_id   BIGINT        REFERENCES board_comments(id),
    author_id   BIGINT        NOT NULL,
    author_name VARCHAR(40)   NOT NULL,
...
ALTER TABLE board_comments DROP CONSTRAINT IF EXISTS ck_board_comment_status;
ALTER TABLE board_comments ADD CONSTRAINT ck_board_comment_status
