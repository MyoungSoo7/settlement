-- V2: deposit-service 자체 DB(lemuel_deposit) 용 Outbox / 멱등 추적 테이블 + ShedLock
--
...
-- (organization V2 / investment V2 / loan V4 / card V2 와 동일 스키마).
--
-- ★ organization 은 이벤트 봉투 표준 컬럼(occurred_at/event_version/producer, DATA-STANDARD N4)을
--   V20260728010000 에서 나중에 백필로 얹었지만, deposit 은 card 와 마찬가지로 신규 서비스라
--   처음부터(occurred_at NOT NULL, 백필 UPDATE 불필요) 포함한다. 리텐션 함수
--   (prune_outbox_published/prune_processed_events)와 ShedLock 테이블도 organization 의
--   하드닝 캠페인 최종 상태를 기준으로 처음부터 포함한다.

CREATE TABLE IF NOT EXISTS outbox_events (
    id                BIGSERIAL PRIMARY KEY,
    aggregate_type    VARCHAR(50)  NOT NULL,      -- 예: "DepositAccount" (KafkaOutboxPublisher.resolveTopic 이 소문자화해 토픽 접두어로 씀 — §3.4/application.yml 참조)
    aggregate_id      VARCHAR(64)  NOT NULL,
    event_type        VARCHAR(100) NOT NULL,      -- 예: "DepositCredited" (aggregate_type prefix 제거 후 snake_case 변환 → lemuel.deposit.credited)
    ...
    CONSTRAINT chk_deposit_outbox_status CHECK (status IN ('PENDING', 'PUBLISHED', 'FAILED'))
);

CREATE INDEX IF NOT EXISTS idx_deposit_outbox_status_created
    ON outbox_events (status, created_at)
    WHERE status IN ('PENDING', 'FAILED');

CREATE UNIQUE INDEX IF NOT EXISTS uq_deposit_outbox_event_id
    ON outbox_events (event_id);

CREATE INDEX IF NOT EXISTS idx_deposit_outbox_aggregate
    ON outbox_events (aggregate_type, aggregate_id);

CREATE INDEX IF NOT EXISTS idx_deposit_outbox_pending_claim
    ON outbox_events (created_at, claimed_at)
    WHERE status = 'PENDING';

-- 사건 시각 기준 조회(지연 측정·기간별 재처리)를 위한 인덱스.
CREATE INDEX IF NOT EXISTS idx_deposit_outbox_occurred_at
    ON outbox_events (occurred_at);

-- PUBLISHED outbox 정리 스캔용 부분 인덱스 (위 idx_deposit_outbox_status_created 는 PENDING/FAILED 전용이라 미커버)
CREATE INDEX IF NOT EXISTS idx_deposit_outbox_published_prune
...
CREATE INDEX IF NOT EXISTS idx_deposit_processed_events_processed_at
...
-- 현재 prod 는 replicas: 1 이라 미래 HA 대비 (hold 만료 스캔·상계 부족분 재시도 배치가 사용).
