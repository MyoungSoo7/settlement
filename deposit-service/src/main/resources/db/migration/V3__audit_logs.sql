...
--
-- deposit-service 는 루트 컴포넌트 스캔으로 shared-common 의 AuditLogJpaEntity(@Table audit_logs) 를
-- 포함하므로, 자체 DB 에도 동일 테이블이 필요하다 (ddl-auto=validate 정합).
--
-- ★ organization 은 비파티션 V3 로 먼저 만들고 V20260717100000 에서 나중에 파티션 부모로 전환
--   (RENAME → 신규 파티션드 부모 생성 → 데이터 이관 → 구 테이블 DROP) 했지만, deposit 은 card 와
--   마찬가지로 신규 서비스라 그 리네임/이관 단계가 필요 없다 — 처음부터 파티션드 부모 + append-only
--   트리거 + 유지보수 함수 2종으로 작성한다 (organization/investment/loan/card 의 최종 표준 상태와 동일 스키마).
