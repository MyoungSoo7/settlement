...
test('pipeline — --help: 사용법 출력 (exit 0)', () => {
  ...
  assert.match(r.stdout, /identity gate -> diagnose-company -> briefing -> briefing-eval/);
});
...
test('pipeline — --agent none: 로컬 브리핑까지 생성하고 채점·DOCX까지 완료 (exit 0)', () => {
  ...
  try {
    ...
    assert.match(r.stdout, /로컬 브리핑 생성/);
    ...
    assert.ok(existsSync(join(out, 'briefing.docx')));
    assert.ok(existsSync(join(out, 'executive-summary.png')));
...
test('pipeline — 가짜 에이전트 전 구간: 게이트→진단→브리핑→채점 EVAL PASS (exit 0)', () => {
  ...
  try {
    ...
    assert.match(r.stdout, /PIPELINE READY/);
    assert.ok(existsSync(join(out, 'briefing.md')));
    assert.match(r.stdout, /\[5\] 스냅샷 PNG \+ Word 보고서 렌더/);
