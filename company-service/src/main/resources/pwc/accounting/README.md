...
| `executive-summary.png` | 진단 패킷의 DART/뉴스/거시 신호를 한 장으로 요약한 CEO용 시각 스냅샷, DOCX에 자동 삽입 | `render-executive-snapshot.py` — **선택 의존**: Python 3 + Pillow 필요. 미설치면 PNG 만 생략되고 파이프라인·DOCX 는 정상 완주 |
...
|   |   |-- signals.mjs             # 내부 신호 파생 엔진 S1~S4 (데이터 → 정답지)
|   |   |-- dart-signals.mjs        # 외부 신호 파생 엔진 E1~E5·E8 (DART·ECOS)
|   |   |-- market-signals.mjs      # 시장 신호 E6·E7 (KRX 시세·거래대금)
|   |   |-- news-signals.mjs        # 뉴스 정성 보조 신호
|   |   |-- docs.mjs                # 비정형 문서 로드 + D1 인젝션 스캐너
|   |   |-- crosscheck.mjs          # INV-8 공시 대사
|   |   |-- judge.mjs               # 2차 LLM Judge (advisory)
|   |   |-- presets.mjs + presets/  # 업종 프리셋 (값마다 rationale 강제)
|   |   |-- docx.mjs                # zero-dependency Word 렌더러
|   |   |-- local-briefing.mjs
|   |   `-- env.mjs
|   |-- bin/
|   |   |-- doctor.mjs                    # 환경 진단 (설치 확인 원커맨드, 네트워크 0)
|   |   |-- ceo-consulting-pipeline.mjs   # 통합 파이프라인 (게이트→진단→브리핑→채점 완주)
|   |   |-- diagnose-company.mjs          # 진단 진입점 (API-only / 상세 / 뉴스·시장 축)
|   |   |-- engagement-cycle.mjs          # 브리핑 이후 반복 컨설팅 상태머신
|   |   |-- quarterly-briefing-batch.mjs  # 분기 브리핑 배치 (목록 일괄 완주 + 문서함 업로드)
|   |   |-- build-briefing-universe.mjs   # 상장사 유니버스 빌더 (배치 목록 생성, DART 보강)
|   |   |-- demo-e2e.mjs                  # 원커맨드 E2E 데모
|   |   |-- calibrate.mjs                 # 임계값 발화율 측정 (코호트 라이브)
|   |   |-- verify-books.mjs              # 불변식 게이트 CLI (추론 전 기계 검증)
|   |   |-- detect-signals.mjs            # 신호 파생 CLI (에이전트/채점기 공용 엔진)
|   |   |-- render-briefing-docx.mjs      # briefing.md → briefing.docx
|   |   |-- run-sample.ps1
|   |   |-- dart-cli.mjs
|   |   `-- dart-to-csv.mjs               # DART 공시 기반 trial_balance_public.csv 생성
|   |-- skills/
|   |   |-- ceo-risk-recon/ · company-identity/ · ceo-briefing/
|   |   |-- accounting-anomaly/ · cashflow-bottleneck/ · cost-allocation-audit/
|   |   |-- macro-exposure/ · disclosure-crosscheck/ · external-signal/
|   |   `-- ceo-engagement-cycle/ · engagement-followup/ · engagement-review/ · engagement-retro/
|   |-- data/sample/                # 데모 데이터 (이상 신호 4종)
|   |   |-- trial_balance.csv
|   |   |-- ar_aging.csv
|   |   `-- cost_allocation.csv
|   |-- data/fixtures/clean/        # 음성 픽스처 (이상 없는 건강한 회사)
|   |-- data/briefing-companies.json  # 분기 배치 기본 대상 목록 (코스피 대형주 20사 큐레이션)
|   |-- dart/
|   |   |-- client.mjs
|   |   |-- corp-codes.mjs
|   |   `-- universe.mjs            # 상장사 전체 → 배치 목록 보강 엔진
|   |-- ecos/
|   |   `-- client.mjs
|   |-- krx/
|   |   `-- client.mjs              # 시장 축 (E6·E7)
|   |-- naver/
|   |   `-- client.mjs
|   |-- registry/
|   |   `-- client.mjs
...
|   |   `-- registry-server.mjs
|   `-- test/
|       |-- unit/                   # 단위 테스트 (네트워크 0, fetch 스텁)
|       |-- dart-smoke.mjs · ecos-smoke.mjs · news-smoke.mjs · registry-smoke.mjs
|       |-- judge-smoke.mjs
...
```bash
codex plugin marketplace add <이 accounting 폴더 경로 또는 repo>
...
- [`docs/poc-trusted-ceo-accounting-agent.md`](docs/poc-trusted-ceo-accounting-agent.md): PoC 제안서
- [`docs/plugin-evaluation-report.md`](docs/plugin-evaluation-report.md): 플러그인 평가 리포트
- [`../../../../../../STATUS.md`](STATUS.md): 구현 현황 정본
