-- V20260715150000: company-service 감사 로그(audit_logs) 신설 — 3인 DB 리뷰 지적 반영
--
-- 설계 근거:
--   company-service 의 문서함(company_documents) 업로드/삭제는 셀러 리소스에 대한 IDOR 민감
--   작업(ADR 0023, JWT ADMIN/MANAGER 게이트)이지만, 현재 자체 DB 에 감사 추적 테이블이 전무했다.
--   "누가 언제 어떤 기업 문서를 올렸/지웠나"를 포스트모템에서 재구성할 수 없다는 것이 리뷰 지적.
--   → order-service V34 / operation-service V3 / loan-service V5 와 동일한 shared-common audit 표준
--     스키마를 company 자체 DB(public)에 신설한다.
--
-- E3 표준(감사성 강화형) 채택:
--   · created_at 월별 RANGE 파티션(2026_01~2027_06 + DEFAULT) — 리텐션은 파티션 DROP 으로 O(1).
...
--
-- ★ 주의(스키마 선행): 본 마이그레이션은 테이블만 만든다. company 는 shared-common 제한 스캔
--   서비스이므로 AuditLogJpaEntity·기록 서비스(감사 로그 쓰기) Java 배선은 후속 작업이다
--   (현재 매핑 엔티티가 없어 ddl-auto=validate 대상도 아님 — 향후 @Import 로 배선 시 검증 편입).
...
CREATE TABLE IF NOT EXISTS audit_logs (
    ...
    resource_type   VARCHAR(50),               -- CompanyDocument / Reputation / SellerLink ...
    resource_id     VARCHAR(64),               -- 문자열 보관 (종목코드·문서 id 등)
