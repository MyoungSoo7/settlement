...
/**
 * 소스 등록/수집 트리거(/admin/commondata/**) 공유 시크릿 게이트.
 *
 * <p>X-Internal-Api-Key 헤더가 {@code app.commondata.internal-api-key} 와 일치해야 통과.
...
public class AdminApiKeyFilter extends OncePerRequestFilter {
    ...
    public AdminApiKeyFilter(@Value("${app.commondata.internal-api-key:}") String apiKey) {
        this.apiKey = apiKey;
    }

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !request.getRequestURI().startsWith("/admin/commondata/");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("app.commondata.internal-api-key 미설정 — /admin/commondata/** 무게이팅 통과 (로컬 전용, 운영 설정 필수)");
