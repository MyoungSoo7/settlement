...
/**
 * common-data-service 자체 최소 보안 설정.
 *
 * <p>이 서비스가 다루는 데이터는 전부 공공데이터포털로 공개된 데이터라 조회(GET)는 무인증이다.
 * 쓰기 경로인 소스 등록/수집 트리거(/admin/commondata/**)는 {@link AdminApiKeyFilter} 가
 * X-Internal-Api-Key 공유 시크릿으로 게이팅한다(market 의 AdminApiKeyFilter 와 동일 시맨틱).
 * shared-common JWT 스택을 쓰지 않는 이유는 CommonDataApplication javadoc 참조.
 */
@Configuration
@EnableWebSecurity
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/common-data/**").permitAll()
                        ...
                        // 소스 등록/수집 트리거 — 인증 프레임워크상 permitAll 이지만 AdminApiKeyFilter 가 게이팅
                        .requestMatchers("/admin/commondata/**").permitAll()
                        .anyRequest().denyAll()
                )
