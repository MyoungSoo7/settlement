...
class PartitionMaintenanceRunnerTest {
    ...
    @Test
    void run_은_연별_ensure_stock_quote_함수에_years_인자로_위임한다() {
        var runner = new PartitionMaintenanceRunner(jdbcTemplate, "public", 1);

        runner.run(null);

        verify(jdbcTemplate).queryForObject("SELECT public.ensure_stock_quote_partition(?)", Integer.class, 1);
    }

    @Test
    void 함수가_없어_예외가_나도_삼켜서_부팅을_막지_않는다() {
        when(jdbcTemplate.queryForObject(anyString(), eq(Integer.class), any()))
                .thenThrow(new RuntimeException("function ensure_stock_quote_partition does not exist"));
        var runner = new PartitionMaintenanceRunner(jdbcTemplate, "public", 1);
    ...
    @Test
    void ensureMonthly_도_연별_ensure_stock_quote_함수에_years_인자로_위임한다() {
        var runner = new PartitionMaintenanceRunner(jdbcTemplate, "public", 1);
