#!/usr/bin/env python3
"""SessionStart hook for the 2004-4-3 task-decomposition harness."""
...
def main() -> int:
    context = """[작업분해 하네스 워크스페이스 (2004-4-3) — 세션 시작 안내]
...
"""
    ...
    for path in [
        ...
        file_checks.append({"path": path, "status": marker(path)})

    context_with_checks = context + "\n[workspace file check]\n" + "\n".join(
        f"  - {item['path']}: {item['status']}" for item in file_checks
    )
    state_dir = WORKSPACE / "state"
    state_dir.mkdir(exist_ok=True)
    (state_dir / "session-start-context.md").write_text(context_with_checks + "\n", encoding="utf-8")
