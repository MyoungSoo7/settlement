...
async function testMcp() {
  ...
  send({ jsonrpc: '2.0', id: 4, method: 'tools/call', params: { name: 'guard_check', arguments: { file_path: 'settlement-service/src/main/java/github/lms/lemuel/settlement/domain/Fee.java', content: 'double fee = amount * 0.035;' } } });
  send({ jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'guard_check', arguments: { command: 'git status' } } });
  ...
  assert('tools/list 15개 도구', list?.result?.tools?.length === 15,
    `got ${list?.result?.tools?.length}`);
  const toolNames = (list?.result?.tools ?? []).map(t => t.name);
  for (const t of ['integrity_check', 'ledger_completeness', 'payout_recon', 'holdback_status',
    'stuck_states', 'refund_adjustments', 'event_accounting', 'guard_check']) {
    assert(`도구 등록: ${t}`, toolNames.includes(t), `missing ${t}`);
  }

  const guardBad = responses.find(r => r.id === 4);
  const gb = guardBad && JSON.parse(guardBad.result.content[0].text);
  assert('guard_check: double 금액 파일 blocked', gb?.blocked === true && gb?.mode === 'file',
    JSON.stringify(gb));
  const guardOk = responses.find(r => r.id === 5);
  const go = guardOk && JSON.parse(guardOk.result.content[0].text);
  assert('guard_check: 일반 명령 통과', go?.blocked === false && go?.violations?.length === 0,
    JSON.stringify(go));
