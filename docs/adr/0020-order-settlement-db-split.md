...
| 5.5 | **opslab 원본 정리(decommission)** | 🟡 준비완료(실행 게이트) | 코드 잔재 제거(order `LemuelApplication` 의 settlement/ledger/payout/chargeback/pgreconciliation 스캔 엔트리 — 클래스 0개의 번들 잔재) + 가드형 제거 스크립트([`settlement-opslab-decommission.sh`](../../scripts/etl/settlement-opslab-decommission.sh): dry-run 기본·settlement_db 대조·CONFIRM=DROP) + [런북](../runbook/settlement-db-decommission.md). **스키마 버그 수정**: 스크립트가 `public.` 을 드롭하던 것을 실제 위치인 `opslab` 스키마로 교정(과거엔 잔여를 전혀 못 지움) → `OpslabDecommissionIT`(Testcontainers)로 "settlement 테이블만 드롭·order 테이블 보존(FK CASCADE 안전)" 회귀 검증. 실제 DROP 은 안정화·롤백창 종료 후 수동 실행 |
| 5.5a | **서빙 경로 opslab 직독 제거** | ✅ 완료 | 검색(`SettlementSearchJdbcRepository`)·셀러 리포트(`CashflowAggregateQueryAdapter`)가 `opslab.orders/payments/users/products` 직조인을 settlement 소유 로컬 프로젝션(`settlement_*_view`)·자체 `settlements` 컬럼으로 컷오버 → settlement_db 단독으로 서빙 성립. stale 잔재(`LedgerOutboxJpaEntity`·`SettlementServiceApplication` 주석) 정리 |
| 5.5b | **대사 self-totals 전환 (cross-DB 직독 완전 제거)** | ✅ 완료 | 대사(일일/기간/PG)가 order DB 를 직접 읽던 것을 제거. order 가 자기 opslab 합계를 내부 API(`/internal/recon/*`, `InternalReconController`+`ReconQueryRepository`)로 노출하고, settlement 는 `OrderReconClient`(RestClient)로 그 값을 받아 자기 settlement_db 숫자와 비교한다 → **양측 모두 자기 DB 만 읽음, cross-DB 연결 0**. cross-DB JOIN(`refunds×settlement_adjustments`)은 분해: settlement 가 자기 adjustments 의 refund_id 집합을 산출 → order 가 그 COMPLETED 합계 반환. **settlement 의 `opslab.` 참조 = 0** (`OpslabReferenceGuardTest` 가 빌드 실패로 강제). 잔여 결합: 대사 배치 경로에 한해 settlement→order **동기 HTTP**(가용성 결합) — 핫패스(정산 생성·조회)는 여전히 무의존. 내부 API 는 gateway 미라우팅(`/internal/**` permitAll). 검증: `OrderReconClientTest`(MockRestServiceServer 계약), 양 서비스 전체 테스트 그린 |
