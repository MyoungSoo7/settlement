...
| 5.5b | **대사 직독 = 의도된 예외 명시** | ✅ 완료 | 대사(일일/기간/PG)는 본질상 order **원천(source of truth)**을 대조해야 하므로 `opslab.*` 직독을 유지하되, [`@AuditCrossRead`](../../settlement-service/src/main/java/github/lms/lemuel/architecture/AuditCrossRead.java)(reason 동봉)로 의도를 코드에 명시. 회귀 가드 `OpslabReferenceGuardTest` 가 **애너테이션 없는 클래스의 `opslab.` 참조를 빌드 실패**로 차단(기존 ArchUnit 의 native-SQL 사각지대 보완). 장기적으로 5.2 메트릭 기반 대사로 대체 가능 |
