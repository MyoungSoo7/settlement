...
| 5.2 | **cross-DB 대사 잡** | ✅ 완료(건수) | 코드 의존성 0 유지 — 양쪽이 행 수 게이지를 노출하고 Prometheus 관측 계층에서 대조. order: `settlement_recon_source_rows{view}`(`SettlementReconciliationGauges`), settlement: `settlement_projection_rows{view}`. 드리프트 알림 `SettlementProjectionDriftHigh`/`Persistent` + 대시보드 드리프트 패널. 자가치유=백필 엔드포인트 재사용. (금액 레벨 대사는 후속) |
...
4. **5.3 Schema Registry** — 이벤트 계약이 여러 컨슈머(loan-service 포함)로 확산될 때 가치 최대. (다음)
5. **5.5 opslab 정리** — 위로 충분한 신뢰가 쌓인 뒤 마지막.
6. **5.2 금액 레벨 대사** — 건수 대사 위에 amount 합계 대조(원천 vs 프로젝션)를 후속 추가.
