...
→ **확정: 해당 구간에 이미 생성된 정산이 있으면 거부**(2026-08-07).

무조건 차단으로 초안을 잡았다가 좁혔다. 정산이 이미 생성된 과거 구간은 스냅샷이라 재계산되지
않으므로(ADR 0004·0014), 소급 등록은 **정산 결과를 바꾸지 못한 채 장부와 정책만 어긋나게** 만든다 —
여기는 막아야 한다. 그러나 계약은 8/1부터인데 등록이 8/7로 늦어진 경우처럼, 그 구간에 <b>아직 정산이
생성되지 않았다면</b> 소급 등록이 장부를 어긋나게 하지 않는다. 무조건 차단하면 이 정상 사례까지 막아
운영자가 DB 를 직접 만지는 쪽으로 샌다.

판정을 날짜가 아니라 **데이터**로 한다: 등록하려는 (scope, 기간)에 걸리는 정산이 한 건이라도 있으면 거부.
...
5. 소급 등록은 **그 구간에 정산이 존재할 때만** 거부(결정 포인트 5) — 날짜가 아니라 데이터로 판정.
