...
> **감사 기준일**: 2026-08-22 · **대상**: order-service(payment) · card-service(VAN) · payment-webhook-service · gateway/nginx
>
...
| `/webhooks/toss` (payment-webhook-service) | HMAC-SHA256 서명 (`internal/webhook/signature.go`) | API Key 없음. 서명 스킴도 Toss 실제 규격이 아닌 일반형(README TODO) |
...
- `/webhooks/toss` — HMAC 서명뿐. 게다가 `payment-webhook-service` 는 `docker-compose.yml` 에 없어
  배포 경로 자체가 미배선이다
