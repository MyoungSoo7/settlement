...
| 실패율 카운터 5종(settlement/payment/order/stock/shipping) 탐지 | 인프라 게이지(kafka lag/redis/deadlock/http) — Alertmanager 룰이 커버 |
...
| `metric-category` | 5종 매핑 | metric_key → SignalCategory. **key 집합이 곧 탐지 대상** |
