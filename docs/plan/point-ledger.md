...
| 3 | ~~관리자 콘솔 `/admin/points` — 잔액·원장 조회, 소멸예정~~ **(2026-08-20 완료)** · 남은 것: 수기 **차감**, 정책 **편집** | 라우트 + 메뉴 2스텝(`menu-route-gate`) |
...
아직 없는 것: **수기 차감**(지급의 역방향 — 오지급 회수 경로)과 **정책 편집 UI**(현재는
마이그레이션으로만 넣는다. ADR 0032 규약상 행 UPDATE 가 아니라 close + 신규 행이라, 편집 화면은
"종료 + 신규 등록" 2단 폼이어야 한다).
