...
**남은 것**: 프론트 체크아웃 UI. 백엔드 경로는 열렸지만 `/payments/split` 을 호출하는 화면이
없다(이번 변경으로 생긴 공백이 아니라 분할결제 API 전체가 그렇다).
...
**하지 않은 것**: 기프트카드 선점(입금 대기 결제에서 거절) · PG 입금 통보 웹훅 연동 ·
포인트 전액 결제의 프론트 체크아웃 UI(§6 ③).
