...
**Phase 1 에서 하지 못한 것**(정직하게 남긴다):

- **account-service GL 3계정·분개 매핑** — 이벤트는 나가지만 아직 소비자가 없다. 즉 포인트 부채가
  시산표에 잡히지 않는다. 다음 커밋 대상.
- **이벤트 계약 스키마·양방향 계약 테스트**(ADR 0024) — 토픽 카탈로그 등록은 마쳤으나
  `contracts/events/` 스키마·정본 샘플은 아직 없다.
