...
1. 원래 소비한 로트가 **아직 살아 있으면**(`ACTIVE`) 그 로트의 `remaining` 을 되돌린다 —
   유효기간이 원래대로 유지된다.
2. 로트가 이미 `EXPIRED`/`EXHAUSTED` 면 **`REFUND_RESTORE` 출처로 새 로트를 발급**하고,
   유효기간은 **원 로트의 잔여 기간이 아니라 정책 기본값**을 적용한다.
   (원 로트 기간을 승계하면 발급 즉시 만료된 포인트를 돌려주는 셈이 된다.)
...
**③ 포인트 100% 결제.** 다른 단일 결제 경로(`payment/application/CreatePaymentUseCase.java`)는
`paymentMethod` 문자열만 받고 **텐더를 모델링하지 않는다** — 여기에 포인트를 얹으면 텐더 개념을
두 번째로 만드는 셈이다. 그래서 `createSplit` 의 "최소 2개" 제약(:59)을 **1개 이상으로 완화**하고
이름을 `createWithTenders` 로 일반화한다. 포인트 전액 결제는 "텐더가 하나인 결제"일 뿐이며,
제약 하나를 지우는 것이 경로를 새로 파는 것보다 훨씬 작은 변경이다.
...
| Phase | 범위 | 완료 판정 |
| --- | --- | --- |
| 1 | 계정·로트·엔트리·소비상세 + 사용/복원 훅 2곳 + 적립정책 + 소멸배치 + 이벤트 5종 + GL 3계정 | `:order-service:test` + JaCoCo LINE 90%, 계약 테스트 GREEN |
