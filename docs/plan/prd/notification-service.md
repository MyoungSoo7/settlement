# PRD — 알림 팬아웃·푸시 (notification-service, 폴리글랏 Kotlin)

> **문서 성격**: 구현된 코드에서 **거꾸로 역산한(reverse-engineered) 제품 요구사항 문서**다.
> 자매 문서 [`settlement-core.md`](settlement-core.md)·[`gateway-service.md`](gateway-service.md) 와 같은 규약을 쓴다 —
...
notification-service 는 **도메인 이벤트 하나를 받아 활성 채널 전부로 동시에 팬아웃하는** 단일 지점이다.
...
| #  | 목표                                     | 성공 기준                                                              |
| -- | ---------------------------------------- | ---------------------------------------------------------------------- |
| G1 | 이벤트 1건이 모든 활성 채널에 동시 도달  | 코루틴 `async`+`awaitAll` 팬아웃, 채널별 독립 타임아웃·재시도          |
...
| G4 | 브라우저가 알림을 즉시 받는다            | `GET /notifications/stream` SSE + `Last-Event-ID` 재개                  |
...
| N3 | `shared-common` 재사용        | 폴리글랏 독립 모듈 — Kafka 에러 핸들링·JWT 검증을 Kotlin 으로 재구현 |
...
| FR    | 요구사항                                                              | 강제 지점                                        |
| ----- | --------------------------------------------------------------------- | ------------------------------------------------ |
| FR-1  | 5토픽을 구독한다                                                      | `DomainEventListener` `@KafkaListener`           |
...
| 메서드 | 경로                     | 인증        | 설명                                       |
| ------ | ------------------------ | ----------- | ------------------------------------------ |
| POST   | `/notifications/send`    | **없음**    | 임의 알림 발송(게이트웨이 미노출)          |
| GET    | `/notifications/demo`    | **없음**    | 샘플 알림 팬아웃(게이트웨이 미노출)        |
| GET    | `/notifications/stream`  | JWT 필수    | SSE 푸시 — 게이트웨이가 노출하는 유일 경로 |
...
| 방향 | 토픽                                                                                                     |
| ---- | -------------------------------------------------------------------------------------------------------- |
| 소비 | `lemuel.settlement.confirmed` · `lemuel.payment.confirmed`(Go 웹훅) · `lemuel.payment.captured` · `lemuel.payment.refunded` · `lemuel.investment.executed` |
...
| NFR-6 | JDK 21 고정                      | 툴체인 21 — JDK25/Kotlin2.3 조합 회피(빌드 시 `JAVA_HOME` 필수)  |
...
`/api/notifications/stream` 에는 없다. G-1·G-2 가 해소돼도 이벤트가 묶여 도착하고 60초마다 끊긴다.
상세는 [`gateway-service.md`](gateway-service.md) G-2.

### G-4. 이벤트 소비가 기본 OFF 다

`app.kafka.enabled` 기본값이 `false` 다. "브로커 없이 기동"이라는 목표(G6)의 대가로, **실배포에서
`APP_KAFKA_ENABLED=true` 를 넣지 않으면 알림의 입력 소스가 없다** — REST `/notifications/send` 만 동작한다.
그리고 그 REST 경로는 게이트웨이에 노출돼 있지 않다(의도적). 즉 기본 구성의 이 서비스는 아무 것도
받지 않고 아무 것도 내보내지 않는다.

### G-5. 무인증 발송 경로가 존재한다

`POST /notifications/send` 와 `GET /notifications/demo` 에는 인증이 **전혀** 없다. 임의 수신자에게 임의 제목·본문
알림을 만들 수 있다. 현재 방어는 "게이트웨이에 라우트를 안 올린다" 하나뿐이라, 서비스 포트(8130)에 직접
도달할 수 있는 위치(같은 네트워크, 포트포워딩, k8s 파드 간)에서는 그대로 열려 있다.
