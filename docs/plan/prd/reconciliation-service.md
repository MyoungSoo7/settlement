...
> 새 기능을 제안하지 않고, 이미 동작 중인 시스템이 _무엇을, 왜, 어떤 규칙으로_ 하는지를 제품 관점으로 재진술한다.
>
> | 항목      | 값                                                                                                                                                                                  |
> | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
### G-3. `app.reconciliation.tolerance-krw` 설정이 읽히지 않는다 — **해소됨 (2026-08-20)**
...
`ReconciliationScheduler` 와 `ReconciliationController` 가 모두 `@Value("\${app.reconciliation.tolerance-krw:1}")`
로 같은 값을 읽도록 배선했다. 배치와 API 가 같은 기준을 쓰게 하는 것이 요점이다 — 대사 판정 기준이 호출
경로에 따라 갈리면 "어느 쪽이 맞는가" 를 사후에 알 수 없다.

기본값이 종전 상수와 같은 `1` 이고 운영 환경에 `APP_RECONCILIATION_TOLERANCE_KRW` 오버라이드가 없으므로
(helm-deploy `charts/polyglot-services/values.yaml` 확인) **현재 판정 결과는 바뀌지 않는다.** 바뀐 것은
이제부터 그 환경변수가 실제로 동작한다는 사실이다.

이 갭은 2026-08-20 고아 파라미터 감사(정의됐지만 읽히지 않는 설정 키 스캔)에서 이 문서와 무관하게 다시
발견됐다 — 같은 결함이 두 경로로 잡혔다는 건 감사 방법이 실제로 작동한다는 신호다.
