# PRD — 조직·멤버십 (organization-service)
...
> | 근거      | 도메인 9개 클래스, 진입 어댑터 1종(REST), 발행 4토픽, 테스트 10개 클래스                |
> | 범위 밖   | 권한 판정(소비측 몫) · 조직 단위 정산 배분 · 사용자 계정 자체(order-service `users`)    |
> | 관련 문서 | [`../../../SPEC.md`](../../../SPEC.md) · [`organization-service-membership.seed.yaml`](../seeds/organization-service-membership.seed.yaml) · `card-service-rules`(소비측) |
