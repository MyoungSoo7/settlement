...
> 자매 문서 [`settlement-core.md`](settlement-core.md)·[`common-data-service.md`](common-data-service.md) 와 같은 규약을 쓴다 —
...
> | 관련 문서 | [`../../../SPEC.md`](../../../SPEC.md) · [`../../inflearn/polyglot-services.md`](../../inflearn/polyglot-services.md) · [`../seeds/gateway-service-routing.seed.yaml`](../seeds/gateway-service-routing.seed.yaml) |
...
1. financial·economics·company·market·commondata 는 **공개 조회 경로만** 라우트에 있다(`/api/{도메인}/**`).
...
| BR-3 | **권한 등급이 다른 경로는 합치지 않는다** — deposit 의 `/api`(읽기)와 `/admin`(잔고 이동)을 한 와일드카드로 묶지 않는다 | deposit 라우트 주석 |
...
|---|---|---|---|
| 1 | `order-service-orders` | 8088 | `/auth`·`/users`·`/orders`·`/payments`·`/coupons`·`/reviews`·`/games`·`/categories`·`/memberships`·`/products` + `/api/{products,categories,tags,payments,menus}` + `/admin/{categories,products,pg,menus,common-codes,rbac,settlement-projection,payment-expiry,stock-reclaim,seller-tiers,shipments,option-catalog,display-sections}` + `/display-sections` |
| 2 | `settlement-service` | 8082 | `/settlements` + `/api/{settlements,ledger,reports,tax-invoices}` + `/api/seller/bank-account` + `/admin/{settlements,payouts,seller-bank-accounts,chargebacks,pg-reconciliation,reconciliation,integrity,recoveries,monthly-closing,outbox,dlq,backfill,event-track,ledger-periods,seller-tax-profiles,tax,commission-rates}` |
| 3 | `loan-service` | 8084 | `/loans/**` |
| 4 | `financial-statements-service` | 8086 | `/api/financial/**` |
| 5 | `economics-service` | 8087 | `/api/economics/**` |
| 6 | `company-service` | 8090 | `/api/company/**` |
| 7 | `operation-service` | 8092 | `/api/ops/**` |
| 8 | `market-service` | 8094 | `/api/market/**` |
| 9 | `ai-service` | 8096 | `/api/ai/**` |
| 10 | `common-data-service` | 8098 | `/api/common-data/**` |
| 11 | `investment-service` | 8100 | `/api/investment/**` |
| 12 | `account-service` | 8102 | `/api/account/**` |
| 13 | `organization-service` | 8104 | `/api/organizations/**` |
