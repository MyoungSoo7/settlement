...
> 상태: **Phase 3 구현 완료**(정의 CRUD + 게시글·댓글 + LIST/GALLERY 스킨 + sanitize + 첨부). Phase 4 는 §9 참조.
...
| 4 | FAQ/QNA 스킨 + 서버 썸네일 생성 + 고아 파일 청소 배치 | §14 의 남은 항목 |
...
- **서버 썸네일 생성 없음** — 갤러리 그리드가 원본 이미지를 CSS 로 줄여 그린다. 이미지가 많아지면
  대역폭이 낭비되므로 목록용 축소본 생성이 필요하다.
- **텍스트·CSV 첨부 불가** — 매직바이트가 없는 형식이라 판정기가 인식하지 못한다. 받으려면 목록을
  넓히는 게 아니라 판정 방법을 따로 정해야 한다.
- **고아 파일 청소 배치 없음** — 삭제 실패·보상 실패로 남은 파일을 주기적으로 훑는 경로가 없다.
