...
**notification-service 에는 그 경로가 없다.** 이 서비스는 자체 DB 가 없는 무영속 standalone(Kotlin/Boot 3.x)이라
...
```bash
# DLT 유입이 멎었는지
curl -s http://localhost:8130/actuator/prometheus | grep notification_kafka_dlt_published_total
# 재발송이 실제로 일어났는지 (dedupe 스킵이면 이 로그가 없다)
docker logs lemuel-notification --since 5m | grep -E "dispatched|dedupe skip"
```
