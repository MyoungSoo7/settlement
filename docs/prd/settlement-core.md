...
> | 관련 문서   | [`SPEC.md`](../../SPEC.md) §3.2 · [`CLAUDE.md`](../../CLAUDE.md) · [`docs/adr/`](../adr/) · `settlement-domain-rules` 스킬 |
...
`docs/STRUCTURE.md` 도메인 목록을 현행화하고, 재발은 `harness-audit` 의 서브도메인 로스터 검사로 기계 차단한다
(코드 디렉터리 ↔ 문서 트리 줄 대조). 같은 작업에서 **모듈 로스터 검사가 `STRUCTURE.md` 를 루트 경로로 조회해
한 번도 검사되지 않던 결함**도 함께 고쳤다(`docs/STRUCTURE.md`).
...
- **대사 clawback 의 원장 역분개 미반영** — `Settlement.applyReconciliationClawback` 주석이 "원장 역분개는 후속 과제"로 명시.
...
| T-3 | 대사 clawback 원장 역분개   | 별도 설계 후 `RECON_REVERSED` 전표 경로 연결                                  |
