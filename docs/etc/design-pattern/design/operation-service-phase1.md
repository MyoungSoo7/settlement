...
# operation-service Phase 3 — 베이스라인 이상 탐지 (anomaly BC)

> 상태: **구현 완료** (2026-07-12). Phase 2 가 쌓은 `ops_metric_bucket` failure_rate 시계열을 입력으로,
> "평소 대비 실패율 급증"을 롤링 z-score 로 판정해 `source=ANOMALY` 인시던트로 승격한다.
> 선행: Phase 2a/2b (신호 버킷). 후속: Phase 4 AI 브리핑.

## 3.1 범위

| 포함 | 제외 |
|------|------|
| 신규 `anomaly` BC (도메인 z-score 판정 + 스케줄 스캐너) | 계절성(요일×시간대) baseline — 인터페이스만 선점 |
| 실패율 카운터 탐지 — 현재 **4종 활성**(settlement/payment/stock/shipping) | 인프라 게이지(kafka lag/redis/deadlock/http) — Alertmanager 룰이 커버 |
| — order 는 `order.failed` 프로듀서 배선 후 활성(§2b.4, count_signal 0 이라 현재 판정 불가) | — |
| `source=ANOMALY` 인시던트 자동 생성/refire/자동해제 | 신규 테이블·마이그레이션 (기존 자산 재사용) |
| 판정 임계 `app.ops.anomaly.*` 전면 외부화 | AI 설명·일일 브리핑 (Phase 4) |

**설계 결정 — 산출물은 기존 인시던트 라이프사이클 재사용**: Phase 1 이 선점한 `IncidentSource.ANOMALY`
enum + `incidents` 테이블(CHECK 에 ANOMALY 포함) + 상태머신 + `uq_incident_active` 부분 유니크 인덱스를
그대로 쓴다. **신규 Flyway 마이그레이션 없음** — 스키마 변경 0.

## 3.2 도메인 (`anomaly.domain`, 순수 POJO)

- `AnomalyThreshold` — z 임계·criticalZ·windowSize·minSampleTotal·failureRateFloor·resolveStreakK (전부 yml 외부화).
- `BaselineStrategy` (인터페이스) + `RollingWindowBaseline`(구현 1종) — 직전 N개 버킷 failure_rate 의 평균·**모표준편차**.
  계절성 전략은 인터페이스로 선점만(히스토리 축적 시 구현체 교체로 확장).
- `AnomalyEvaluator` — 판정 순수 함수. **3중 게이트를 AND** 로 통과할 때만 이상:
  1. **퇴화 게이트** — stddev=0(또는 표본 0) → 정상 (0 나눗셈 원천 차단)
  2. **최소표본** — count_total < minSampleTotal → 정상 (저표본 튄 실패율 오탐 차단)
  3. **상대임계 하한** — failure_rate < failureRateFloor → 정상 (절대적으로 낮은 실패율은 이상 아님)
  4. **z 임계** — z = (failure_rate − mean)/stddev < zThreshold → 정상
- `AnomalyDecision` — verdict·z·baseline·critical(z≥criticalZ)·reason(인시던트 description 재료).

## 3.3 실행 모델 (`anomaly.application` + `adapter`)

- `AnomalyScanScheduler` — `@ConditionalOnProperty(app.ops.anomaly.enabled)` + `@Scheduled(fixedDelay 5분)`.
  Prometheus 폴러와 동일 패턴(로컬/테스트 기본 off, 단일 스레드 비겹침).
- `AnomalyDetectionService` (오케스트레이터, tx 없음) — metric 당:
  1. 마감된 버킷(`bucket_start < floor(now)`)을 `windowSize + resolveStreakK` 개까지 시간순 조회
     (`MetricSeriesQueryAdapter` 가 signal 의 `ops_metric_bucket` 리포지토리를 읽기 재사용).
  2. 히스토리 `windowSize + 1` 미만이면 스킵(콜드스타트).
  3. **직전 마감 버킷 1개**를 롤링 베이스라인 대비 판정.
  4. 이상이면 생성/refire, 정상이면서 **직전 K개 버킷이 모두 정상**이면 자동 해제.
  - 한 metric 실패가 다른 metric·다음 스캔을 막지 않음(건별 try). 동시 경쟁(중복 INSERT/낙관적 락)은
    새 트랜잭션 재시도로 refire 수렴(최대 5회) — Alertmanager `IngestAlertService` 와 동일 패턴.
- `AnomalyIncidentApplier` (`@Transactional`, 별도 빈) — metric 1건 = 1 트랜잭션 경계.
  incident BC 의 `Incident.openFromAnomaly` / `refire` / `autoResolve(ANOMALY_ACTOR, …)` + ports 재사용.
  - **correlation_key = metric_key**, `source=ANOMALY` → `uq_incident_active` 로 metric 당 활성 1건.
    Alertmanager 경로(correlation=fingerprint)와 key 공간 분리 → 채널 간 충돌 0.
  - **severity**: `z ≥ criticalZ(5.0)` → CRITICAL, 그 외 WARNING. refire 는 승격만 반영(기존 규칙).
  - **자동 해제**: 정상 K회 연속 → `AUTO_RESOLVED`, `resolved_by='anomaly-detector'`.

## 3.4 설정 (`app.ops.anomaly.*`, 재배포 없이 튜닝)

| 키 | 기본값 | 의미 |
|---|---|---|
| `enabled` | false | 스캐너 빈 토글 (`OPS_ANOMALY_ENABLED`) |
| `scan-interval-ms` | 300000 | 스캔 주기 (버킷 폭과 일치) |
| `window-size` | 12 | 롤링 베이스라인 버킷 수 (1시간) |
| `z-threshold` / `critical-z-threshold` | 3.0 / 5.0 | 이상 z 하한 / CRITICAL 승격 z 하한 |
| `min-sample-total` | 30 | 최소 표본(count_total) 게이트 |
| `failure-rate-floor` | 0.10 | 상대임계 하한 |
| `resolve-streak-k` | 3 | 정상 연속 자동해제 조건 |
| `metric-category` | 4종 매핑 | metric_key → SignalCategory. **key 집합이 곧 탐지 대상**. order 는 order.failed 배선 후 추가 |

## 3.5 테스트 (16건, 커버리지 게이트 90% 통과)

| 레벨 | 대상 | 핵심 |
|---|---|---|
| 도메인 | `AnomalyEvaluatorTest`(7)·`RollingWindowBaselineTest`(3) | z-score·3중 게이트·severity·경계값·0나눗셈, 평균·모표준편차 |
| 서비스 | `AnomalyDetectionServiceTest`(3, Mockito) | 급증→생성·히스토리부족 스킵·정상연속 해제자격 |
| 통합 | `AnomalyDetectionIntegrationTest`(3, Testcontainers) | **합성 버킷 백테스트**: 스파이크만 탐지·정상 미탐·정상복귀 AUTO_RESOLVED |
| 아키텍처 | `OperationArchitectureTest` | 신규 `anomaly` BC 도 헥사고날·경계 규칙 준수 |

## 3.6 구현 노트

- **마이그레이션 없음**: incidents 의 `chk_incident_source` 가 이미 ANOMALY 를 허용하고, `ops_metric_bucket`
  + `idx_metric_bucket_recent(metric_key, bucket_start DESC)` 를 그대로 쓴다. incident 도메인엔
  `openFromAnomaly` 팩토리 + `autoResolve(actor, at)` 오버로드만 추가(Alertmanager 경로 무영향).
- **콜드스타트 대응**: 버킷 적재는 Phase 2(2026-07-07) 시작이라 요일 계절성 baseline 은 히스토리 부족.
  이번 컷은 롤링윈도우로 출시하고 계절성은 `BaselineStrategy` 로 선점 — 데이터가 쌓이면 구현체 교체.
- **모표준편차 채택**: 윈도우를 "정상 구간 모집단"으로 보고 표본표준편차(n-1) 과대추정에 의한 임계 흔들림을 피함.
- **정상복귀 판정의 무상태성**: 자동해제 스트릭을 별도 상태로 저장하지 않고, 직전 K개 버킷을 각자 자기
  베이스라인으로 재판정해 K회 연속 정상을 확인한다(버킷은 append-only 라 재계산이 항상 결정론적).

## 3.7 로컬 실기동 검증 (2026-07-12)

`OPS_ANOMALY_ENABLED=true` + 10초 스캔으로 실 DB(lemuel_operation)에 합성 버킷을 주입해 관측:
- 정상 baseline 12버킷(≈1~3%) + 스파이크(45%) → **`OPEN / CRITICAL / SETTLEMENT_FAILURE`** 자동 생성
  (`z=52.66`, baseline 0.02±0.0082), 연속 스캔에서 **REFIRED**(occurrence_count 증가, 활성 1건 유지).
- 버킷을 정상 시계열로 교체 → 다음 스캔에서 **`AUTO_RESOLVED`**(`resolved_by='anomaly-detector'`).
- 타임라인 `OPENED → REFIRED → AUTO_RESOLVED` 전 구간 관측. 검증 후 합성 데이터 정리.
