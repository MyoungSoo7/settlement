...
> **현행화 기준: 2026-07-06** — ADR 0020(이벤트 드리븐 프로젝션 + DB-per-service 물리 분리 완료),
> SKU 재고 조건부 원자 UPDATE, 환불 READ_COMMITTED+비관 락 기준으로 작성됨.
> 아키텍처가 바뀌면 이 문서도 함께 갱신할 것 (README·ADR 과 불일치하면 면접에서 자기모순이 된다).
...
주문/결제와 정산은 Bounded Context가 명확히 다릅니다. 주문/결제는 실시간 사용자 요청 중심이고, 정산은 배치/비동기 처리 중심이라 배포 주기와 스케일링 요구가 다릅니다. 분리 기준은 DDD의 Bounded Context로 삼았고, order-service(주문/결제/상품/유저), settlement-service(정산/원장/지급/대사/리포트), loan-service(선정산 대출), financial-statements-service(재무제표 조회) 4개 서비스 + API Gateway로 나눴습니다. **4개 서비스 모두 DB-per-service**(opslab / settlement_db / lemuel_loan / lemuel_financial)로 물리 분리했고, 서비스 간 연계는 Kafka 이벤트로만 합니다. settlement가 order 데이터를 필요로 할 때는 order가 발행한 이벤트를 자체 DB의 프로젝션 테이블(`settlement_*_view`)에 적재해 조회하는 이벤트 드리븐 CQRS(ADR 0020)를 씁니다. `settlement-service/build.gradle.kts`에 `implementation(project(":order-service"))`가 없어 코드 의존 0, DB도 분리라 데이터 의존 0입니다.

### Q2. 서비스 간 데이터 조회를 이벤트 드리븐 프로젝션으로 해결한 이유는? 동기 API·공유 DB와 비교하면?

MSA에서 서비스 간 데이터 조회는 보통 동기 API 호출, 이벤트 기반 데이터 복제, 공유 DB 세 가지 선택지가 있습니다. 동기 API는 settlement-service가 order-service에 런타임 의존성을 갖게 되어 장애 전파 위험이 있고, 공유 DB는 스키마 결합으로 독립 배포를 막습니다. 이 프로젝트는 두 단계로 진화했습니다. 과도기에는 같은 PostgreSQL을 공유하며 `@Immutable` 읽기 전용 엔티티로 매핑하는 Read-only Projection을 썼지만(정합성 우선), 최종적으로 ADR 0020에서 **이벤트 드리븐 프로젝션**으로 전환해 DB까지 물리 분리했습니다. settlement가 자체 DB(settlement_db)에 프로젝션 테이블(`settlement_order_view`, `settlement_payment_view` 등)을 소유하고, order가 Outbox+Kafka로 발행한 이벤트(`order.created`, `payment.captured/refunded` 등)를 컨슈머가 로컬에 적재합니다. Eventually Consistent 리스크는 (1) 핫패스인 정산 생성에 필요한 값(amount·sellerTier 등)을 이벤트에 동봉하는 Event-Carried State Transfer로 프로젝션 lag과 무관하게 만들고, (2) 일일 대사가 order의 내부 API(`/internal/recon`, 공유 시크릿 인증)를 호출해 양측 합계를 비교하는 안전망으로 상쇄합니다 — 이때도 양측 모두 자기 DB만 읽으므로 cross-DB 연결은 0입니다.

### Q3. Transactional Outbox 패턴의 동작 원리와 도입 배경은?

결제 CAPTURED 시점에 Kafka로 이벤트를 발행해야 하는데, DB 커밋과 Kafka 발행을 같은 트랜잭션에 묶을 수 없습니다. 커밋 후 발행하면 발행 실패 시 이벤트 유실, 발행 후 커밋하면 커밋 실패 시 고스트 이벤트가 발생합니다. Outbox 패턴은 도메인 트랜잭션 안에서 `outbox_events` 테이블에 PENDING 상태로 INSERT하고, 별도 폴러(기본 2초 주기, 멀티워커가 `FOR UPDATE SKIP LOCKED`로 나눠 claim)가 이를 읽어 Kafka로 발행 후 PUBLISHED로 전이합니다. DB 커밋 성공 = 이벤트 영속화 성공이므로 원자성이 보장됩니다. At-least-once 보장이므로 컨슈머 측 멱등 처리가 필수입니다.
...
### Q6. SKU 재고 차감 동시성은 어떻게 처리했나요? 낙관/비관 락과 비교하면?

**단일 조건부 원자 UPDATE**로 처리합니다. `SpringDataProductVariantRepository.decreaseStockIfAvailable`이 `UPDATE ... SET stock = stock - :qty WHERE id = :id AND stock >= :qty` 한 문장으로 "재고 검증 + 차감 + 매진 전이"를 DB row 락 안에서 원자적으로 수행하고, 영향 행이 0이면 재고 부족으로 처리합니다. `@Version` 낙관 락 + 재시도는 핫딜처럼 같은 SKU에 차감이 폭주하면 충돌·재시도가 누적되어 처리량이 무너지고, `SELECT FOR UPDATE` 비관 락은 락 대기 행렬이 생깁니다. 조건부 UPDATE는 조건 평가가 락 안에서 일어나므로 **충돌 재시도 자체가 없고** 정확히 보유 수량만큼만 성공합니다(초과판매 방지). bulk JPQL UPDATE는 `@Version`을 자동 증가시키지 않으므로 명시적으로 +1 하고, 1차 캐시 stale 방지를 위해 `clearAutomatically=true`를 씁니다. `VariantStockConcurrencyIT`에서 100스레드/재고 50개 시나리오로 정확히 50건 성공, 50건 InsufficientStock, 최종 재고 0(음수 없음)을 실 PG로 검증합니다.

### Q7. 부분 환불에서 Pessimistic Lock + Idempotency Key를 사용하는 이유는?

부분 환불은 동일 결제에 대해 여러 번 호출될 수 있고, 환불 금액 누적이 원 결제 금액을 초과하면 안 됩니다. `RefundPaymentUseCase`는 `READ_COMMITTED` 격리에서 결제 레코드를 `SELECT ... FOR UPDATE` 비관 락으로 잠가 읽고(락 획득 시점의 최신 커밋값 — REPEATABLE_READ면 스냅샷 밖 갱신이 보이지 않아 오히려 위험), `refundableAmount`를 검증한 뒤 PG 환불을 호출합니다. Idempotency Key는 전액 환불 시 `payment-{id}-full`로 자동 생성되고, 부분 환불 시에는 호출자가 반드시 지정해야 합니다(없으면 `MissingIdempotencyKeyException`). `loadRefundPort.findByPaymentIdAndIdempotencyKey`로 이미 COMPLETED된 동일 키의 Refund가 있으면 PG 재호출 없이 현재 상태를 반환합니다.
...
MSA 분리 시 settlement-service와 order-service 간 의존을 끊는 방식과 시점이었습니다. 처음에는 모놀리스에서 settlement 코드가 order/payment 엔티티를 직접 import하고 있었는데, 단숨에 이벤트 기반 복제로 가면 정산 정합성 리스크가 컸습니다. 그래서 **Strangler 방식의 2단계 전환**을 택했습니다. 1단계는 같은 테이블을 `@Immutable` 엔티티로 읽기 전용 매핑하는 Read-only Projection — 코드 의존 0 + Strong Consistency를 먼저 확보하고, 그동안 Outbox+Kafka 파이프라인과 멱등 컨슈머를 성숙시켰습니다. 2단계(ADR 0020)에서 settlement가 자체 DB(settlement_db)에 소유하는 이벤트 드리븐 프로젝션으로 전환해 DB까지 물리 분리했습니다. Eventually Consistent로 바뀌며 생기는 리스크는 이벤트에 정산 필수 값을 동봉(Event-Carried State Transfer)하고 내부 대사 API(`/internal/recon`)로 합계를 검증하는 안전망으로 상쇄했습니다. "한 번에 완벽한 분리"가 아니라 "정합성을 유지한 채 되돌릴 수 있는 단계로 쪼갠 것"이 핵심 전략이었습니다.
...
N+1 문제는 연관 엔티티를 LAZY 로딩으로 조회할 때, 부모 1건 조회 후 자식 N건을 개별 쿼리로 가져오는 현상입니다. Settlement 프로젝트에서 주문 목록 조회 시 각 주문의 결제 정보를 가져올 때 발생할 수 있습니다. 해결 방법은 `@EntityGraph`나 `JOIN FETCH`로 한 번에 가져오기, `@BatchSize`로 IN 절 묶기, DTO Projection으로 필요한 컬럼만 조회하기가 있습니다. settlement-service의 이벤트 드리븐 프로젝션 뷰(`settlement_*_view`)는 정산에 필요한 컬럼만 비정규화해 로컬에 적재하므로 연관 탐색 자체가 없어 N+1 위험이 근본적으로 줄어듭니다.

### Q22. Spring의 트랜잭션 전파(Propagation) 유형 중 REQUIRES_NEW는 언제 사용하나요?

REQUIRES_NEW는 기존 트랜잭션과 독립적인 새 트랜잭션을 시작합니다. 이 프로젝트의 대표 사례는 분할결제 환불입니다. `TenderRefundExecutor.refundTender`가 tender(포인트/상품권/카드) 1건마다 `REQUIRES_NEW`로 독립 커밋하는데, 예전처럼 루프 전체를 한 트랜잭션으로 감싸면 중간 tender의 PG 환불 실패 시 **이미 PG에서 환불된 앞선 tender의 DB 상태까지 롤백**되어 DB-PG 정합성이 깨졌기 때문입니다. 또 PG 환불 실패 이력을 남기는 `RefundLifecycle.fail`도 REQUIRES_NEW라, 본 트랜잭션이 롤백돼도 실패 기록(재시도 근거)은 살아남습니다. 주의할 점은 REQUIRES_NEW 트랜잭션이 커밋되면 외부 트랜잭션이 롤백돼도 그 결과가 유지된다는 것 — 그래서 보상 트랜잭션·감사 로깅처럼 "바깥 성패와 무관하게 남아야 하는" 작업에 적합하고, 반대로 외부와 원자적이어야 하는 작업에 쓰면 정합성 버그가 됩니다. 같은 클래스 내부 호출은 프록시를 타지 않으므로 반드시 별도 빈에서 호출해야 합니다.
...
AOP는 횡단 관심사(로깅, 트랜잭션, 보안)를 비즈니스 로직에서 분리합니다. Settlement 프로젝트에서는 `shared-common`의 감사(Audit) 모듈이 AOP로 PII 마스킹 + 감사 로그를 처리합니다. `@Transactional`도 AOP 기반으로, 프록시가 메서드 호출을 가로채 트랜잭션을 시작/커밋/롤백합니다. 주의할 점은 같은 클래스 내부 메서드 호출 시 프록시를 거치지 않아 AOP가 동작하지 않는 것입니다. 이를 self-invocation 문제라 하며, 별도 빈으로 분리하거나 `AspectJ` 위빙으로 해결합니다. SKU 재고 차감의 재시도 로직에서 `REQUIRES_NEW`가 동작하려면 반드시 다른 빈에서 호출해야 합니다.

### Q26. `@Transactional`의 isolation level을 REPEATABLE_READ로 설정하면 어떤 효과가 있고, 환불에는 왜 쓰지 않았나요?

REPEATABLE_READ는 트랜잭션 시작 시점의 스냅샷을 유지하여 같은 쿼리를 반복 실행해도 동일한 결과를 보장합니다(PostgreSQL은 SSI에 가깝게 동작하며 충돌 시 SerializationFailure를 던짐). 그런데 `RefundPaymentUseCase`는 오히려 **READ_COMMITTED + `SELECT ... FOR UPDATE` 비관 락** 조합을 씁니다. 환불은 "락을 잡은 시점의 최신 커밋값(누적 환불액)"을 읽고 검증해야 하는데, REPEATABLE_READ에서는 락 대기 후 재개해도 트랜잭션 시작 시점 스냅샷 밖의 갱신이 보이지 않아 stale한 누적액으로 초과 환불을 통과시키거나 UPDATE가 0건 처리될 수 있기 때문입니다. READ_COMMITTED에서는 FOR UPDATE가 최신 커밋 행을 반환하므로 "잠금 → 최신값 검증 → 갱신"이 직렬화됩니다. 즉 격리 수준은 높을수록 좋은 게 아니라, 잠금 전략과 조합해 시나리오별로 골라야 한다는 것이 이 사례의 교훈입니다.
...
JPA가 UPDATE 쿼리에 `WHERE version = :currentVersion`을 추가하고, 영향받은 행이 0이면 `OptimisticLockException`을 던집니다. 이는 다른 트랜잭션이 먼저 해당 행을 수정하여 version이 증가했다는 의미이며, 보통 새 영속성 컨텍스트에서 최신 데이터를 다시 읽어 재시도합니다. 다만 이 프로젝트의 SKU 재고 차감은 낙관 락 재시도 방식에서 **조건부 원자 UPDATE**(`WHERE stock >= :qty`)로 전환해 충돌·재시도 자체를 없앴습니다 — 핫딜처럼 경합이 상시적인 지점에서는 "충돌은 드물다"는 낙관 락의 전제가 깨지기 때문입니다. 이때 주의할 점 하나를 배웠는데, bulk JPQL UPDATE는 `@Version`을 자동 증가시키지 않으므로 쿼리에서 `version = version + 1`을 명시해 엔티티 경로의 낙관 락과 정합을 유지하고, `clearAutomatically=true`로 1차 캐시 stale을 방지해야 합니다.

### Q32. Flyway와 Liquibase의 차이, 그리고 마이그레이션 관리 전략은?

Flyway는 SQL 기반으로 단순하고, Liquibase는 XML/YAML/JSON으로 DB 독립적 마이그레이션을 지원합니다. Settlement에서 Flyway를 선택한 이유는 PostgreSQL 전용이므로 DB 독립성이 불필요하고, 순수 SQL로 작성하면 DBA 리뷰가 용이하기 때문입니다. DB-per-service라 마이그레이션도 서비스별로 소유하며, order-service 기준 70개(초기 `V1~V50` 순번 + 이후 `V{timestamp}__` 타임스탬프 명명 혼재 — 신규는 타임스탬프 권장, 브랜치 병합 시 순번 충돌 방지)로 테이블 생성, 인덱스 추가, 컬럼 변경이 모두 버전 관리됩니다. 주의할 점은 이미 적용된 마이그레이션은 수정하면 안 되고(체크섬 검증 실패), 롤백은 별도 마이그레이션으로 작성해야 합니다. CI에서 Testcontainers로 마이그레이션을 매번 처음부터 실행하여 무결성을 검증합니다.
...
핵심 병목은 동일 좌석에 대한 동시 예매입니다. (1) 좌석 선택: Redis 분산 락(`seat:{eventId}:{seatNo}`)으로 동시 접근 직렬화, 락 TTL로 크래시 안전성 확보. (2) 결제: 락 획득 후 결제 진행, 결제 실패 시 락 해제하여 다른 사용자에게 기회 제공. (3) 대기열: 트래픽 폭주 시 Redis Sorted Set 기반 대기열로 유입량 제어. (4) 좌석 상태: DB는 최종 정합성 보장, Redis는 실시간 좌석 현황 표시. global-seat-ticketing 프로젝트에서 Redisson 기반 분산 락으로 구현했으며, 좌석은 재시도 의미가 없으므로(이미 다른 사람이 예매) 즉시 실패(fail-fast) 전략이 적합합니다 — Lemuel SKU 재고의 조건부 UPDATE(영향 행 0 = 즉시 실패)와 같은 철학입니다.

### Q38. "이벤트 드리븐 주문 파이프라인을 설계하라"

주문 생성 → 결제 승인 → 재고 차감 → 배송 준비 → 정산 생성이 이벤트로 연결됩니다. 각 단계는 Transactional Outbox로 이벤트를 발행하여 DB 커밋과 이벤트 발행의 원자성을 보장합니다. 실패 처리는 두 가지 전략: 보상 트랜잭션(Saga)과 재시도입니다. 결제 실패는 주문 취소(보상), 재고 부족은 조건부 원자 UPDATE의 영향 행 0으로 즉시 실패(재시도 무의미), 정산 실패는 FAILED 상태 후 수동/자동 재시도. 멱등성은 각 컨슈머의 `processed_events` 테이블 + 도메인 UNIQUE 제약으로 보장합니다. Settlement 프로젝트의 결제→정산 파이프라인이 이 패턴의 실제 구현입니다.
...
(1) 이중 쓰기(Dual Write): 새 스키마와 구 스키마에 동시 쓰기, 읽기는 구 스키마. (2) 백필(Backfill): 배치로 구 데이터를 새 스키마로 복사, chunk 단위로 진행하여 DB 부하 분산. (3) 전환: 읽기를 새 스키마로 전환, 검증 후 구 스키마 쓰기 중단. (4) 정리: 구 스키마 삭제. Settlement의 43개 Flyway 마이그레이션 중 SellerTier 도입(V32)이 이 패턴을 따랐습니다. `commissionRate` 컬럼 추가 시 기존 데이터는 기본 3%를 유지하고, 신규 생성분부터 등급별 rate를 적용하는 점진적 전환을 했습니다.
...
개발 환경에서 Outbox 폴러의 폴링 주기(2초)와 Kafka 컨슈머의 처리 속도 불균형으로 PENDING 이벤트가 적체되는 상황을 경험했습니다. 원인은 settlement-service의 정산 생성 로직에서 프로젝션 뷰 조회 시 N+1 쿼리가 발생하여 컨슈머 처리가 느려진 것이었습니다. `OutboxPendingBacklog` 메트릭이 알림 임계값을 넘어 감지했고, 즉시 컨슈머 측 쿼리를 JOIN FETCH로 최적화하여 해결했습니다. 이후 대사(Reconciliation)의 3번 불변식(Outbox 발행 수 == 정산 생성 수)으로 이벤트 누락을 자동 감지하는 안전망을 강화했습니다.

### Q47. 기술적 의견 충돌이 있었던 경험은?

MSA 분리 시 서비스 간 데이터 조회 방식에 대해 동기 API 호출 vs DB 기반 Projection으로 의견이 나뉘었습니다. API 호출 방식은 깔끔한 분리이지만 런타임 의존성과 장애 전파 위험이 있고, DB 기반은 정합성과 성능이 우수하지만 DB 공유라는 제약이 있습니다. 트레이드오프를 정리해 당시 우선순위(정산 정합성 > 완전한 분리)를 기준으로 우선 Read-only Projection(`@Immutable` 공유 테이블 매핑)을 선택하되, 이벤트 기반 전환 경로(Outbox+Kafka)를 함께 구축해 두는 것으로 합의했습니다. 이 전환 경로 덕분에 이후 ADR 0020에서 이벤트 드리븐 프로젝션 + DB 물리 분리(settlement_db)로 실제 전환을 완료했고, 두 결정 모두 ADR로 근거를 기록해 "당시에는 왜 그 선택이 옳았는지"를 추적할 수 있게 했습니다.
