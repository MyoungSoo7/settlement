...
### Q6. SKU 재고 차감에 Optimistic Lock을 선택한 이유는? Pessimistic Lock과 비교하면?

Optimistic Lock은 실제 충돌 시에만 재시도 비용이 발생하므로 대부분의 SKU에서는 락 대기 없이 즉시 처리됩니다. Pessimistic Lock(`SELECT FOR UPDATE`)은 단순하지만 동시성이 낮은 환경에서도 항상 락 대기가 발생해 처리량이 제한됩니다. 현재 구현은 JPA `@Version` + 최대 5회 재시도(지수 백오프 10ms~160ms) + `REQUIRES_NEW` 트랜잭션(1차 캐시 stale 회피)입니다. `VariantStockConcurrencyIT`에서 100스레드/재고 50개 시나리오로 정확히 50건 성공, 50건 실패, 음수 재고 0건을 검증합니다. 만약 retry/success 비율이 1.0을 넘는 hot SKU가 발생하면 Redis 분산 락으로 격상하는 것을 메트릭 기반으로 판단합니다.
...
JPA가 UPDATE 쿼리에 `WHERE version = :currentVersion`을 추가하고, 영향받은 행이 0이면 `OptimisticLockException`을 던집니다. 이는 다른 트랜잭션이 먼저 해당 행을 수정하여 version이 증가했다는 의미입니다. Settlement의 SKU 재고 차감에서는 이 예외를 잡아서 최대 5회 재시도합니다. 재시도 시 `REQUIRES_NEW` 트랜잭션으로 새 영속성 컨텍스트를 열어 최신 데이터를 다시 읽습니다. 지수 백오프(10ms~160ms)를 적용하여 동시 충돌 확률을 줄이고, 한계 초과 시 `StockConcurrencyException`을 던져 `variant.stock.decrease.failure` 메트릭을 기록합니다.
